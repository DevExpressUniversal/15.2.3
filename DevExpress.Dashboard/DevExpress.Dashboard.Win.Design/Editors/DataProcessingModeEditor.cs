...
namespace DevExpress.DashboardWin.Design {
	public class DataProcessingModeEditor : ListSelectorEditor {
		...
		public override object EditValue(ITypeDescriptorContext context, IServiceProvider provider, object value) {
			...
			if(dataSource != null) {
				...
				if(newValue.Equals(value))
					return value;
				DataProcessingMode newDataProcessingMode = (DataProcessingMode)newValue;
				bool newIsSqlServerMode = newDataProcessingMode == DataProcessingMode.Server;
				if(newIsSqlServerMode == true || dataSource.DataProcessingMode == DataProcessingMode.Client)
					return newValue;
				SelectedContextService selectedContextService = context.GetService(typeof(SelectedContextService)) as SelectedContextService;
				DashboardDesigner designer = selectedContextService.Designer;
				if (!DataSourceExecutor.ShouldChangeDataProcessingMode(designer))
					return value;
				return newValue;
			}
			else {
