...
namespace DevExpress.DashboardWin.Design {
	class DragDropService : IDragDropService {
		IDesignerHost designerHost;
		public DragDropService(IDesignerHost designerHost) {
			this.designerHost = designerHost;
		}
		public IDragHandler GetDragHandler(IDataObject data) {
			if (data != null && DragHandlerBase.GetToolboxItem(data, designerHost) != null)
				return CreateToolboxItemDragHandler();
			else
				return new DragHandlerBase(designerHost);
		}
		protected virtual ToolboxItemDragHandler CreateToolboxItemDragHandler() {
			return new ToolboxItemDragHandler(designerHost);
		}
	}
}
