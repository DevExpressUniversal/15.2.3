...
namespace DevExpress.DashboardCommon.Service {
	public class ExportOperation : SessionOperation<ExportArgs> {
		public ExportOperation(IDashboardServer server, IDashboardServiceAdminHandlers admin, ExportArgs args)
		...
		protected override IList<AffectedItemInfo> ExecuteInternal(bool dataBindingApplied, DashboardServiceResult result, DashboardServerResult serverResult, DashboardSessionState sessionState) {
			...
			if (Args.Exporter != null)
				Args.Exporter.Export(Args.Stream, Args.ExportInfo.Mode, data, Args.ExportInfo.ExportOptions);
			else
