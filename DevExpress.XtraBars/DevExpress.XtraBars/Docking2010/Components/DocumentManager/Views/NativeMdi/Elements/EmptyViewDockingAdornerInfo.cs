...
namespace DevExpress.XtraBars.Docking2010.Views.NativeMdi {
	class EmptyViewDockingAdornerInfo : BaseElementInfo, IDockingAdornerInfo {
		public EmptyViewDockingAdornerInfo(NativeMdiView view)
			: base(view) {
		}
		protected NativeMdiView View {
			get { return Owner as NativeMdiView; }
		}
		...
		public void UpdateDocking(Adorner adorner, Point point, BaseDocument dragItem) {
			DockingAdornerInfoArgs args = DockingAdornerInfoArgs.EnsureInfoArgs(ref DockingAdornerInfo, adorner, Owner, View.Bounds);
			args.Adorner = Owner.Manager.GetDockingRect();
			args.Container = Owner.Manager.Bounds;
			args.Bounds = View.Bounds;
			args.Content = Rectangle.Empty;
