...
namespace DevExpress.DataAccess.Native {
	public class CriteriaPatcher : IClientCriteriaVisitor<CriteriaOperator> {
		protected CriteriaOperator Process(CriteriaOperator op) {
			return ReferenceEquals(op, null) ? null : op.Accept(this);
		}
		IEnumerable<CriteriaOperator> ProcessAny(IEnumerable<CriteriaOperator> operands) {
		...
		IEnumerable<CriteriaOperator> ProcessEvery(IEnumerable<CriteriaOperator> operands) {
			List<CriteriaOperator> ops = new List<CriteriaOperator>();
			foreach(CriteriaOperator operand in operands) {
				CriteriaOperator op = Process(operand);
				if(ReferenceEquals(op, null))
		...
		CriteriaOperator ICriteriaVisitor<CriteriaOperator>.Visit(InOperator theOperator) {
			...
			if(ReferenceEquals(left, null))
				return null;
			IEnumerable<CriteriaOperator> operands = ProcessAny(theOperator.Operands);
			return operands != null ? new InOperator(left, operands) : null;
		}
