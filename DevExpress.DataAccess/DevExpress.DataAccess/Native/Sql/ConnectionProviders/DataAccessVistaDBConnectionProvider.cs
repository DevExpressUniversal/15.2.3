...
namespace DevExpress.DataAccess.Native.Sql.ConnectionProviders {
	public class DataAccessVistaDBConnectionProvider : VistaDBConnectionProvider, IAliasFormatter, ISupportOrderByExpressionAlias, IDataStoreEx, IDataStoreSchemaExplorerEx {
		...
		public static IDataStore DataAccessCreateProviderFromString(string connectionString, AutoCreateOption autoCreateOption, out IDisposable[] objectsToDisposeOnDisconnect) {
			...
			objectsToDisposeOnDisconnect = new[] {connection, (IDisposable)result};
			return result;
		}
		public static IDataStore DataAccessCreateProviderFromConnection(IDbConnection connection, AutoCreateOption autoCreateOption) {
			return new DataAccessVistaDBConnectionProvider(connection, autoCreateOption);
		}
		static DataAccessVistaDBConnectionProvider() {
			Register();
			RegisterDataStoreProvider(XpoProviderTypeString, DataAccessCreateProviderFromString);
			RegisterDataStoreProvider("VistaDB.Provider.VistaDBConnection", DataAccessCreateProviderFromConnection);
			RegisterFactory(new DataAccessVistaDBProviderFactory());
		}
		public static void ProviderRegister() {
		}
		public DataAccessVistaDBConnectionProvider(IDbConnection connection, AutoCreateOption autoCreateOption)
			...
			foreach(DBTable table in tables) {
				...
				try {
					...
					foreach(XPVistaDBRelationshipInformation fk in vtableGetForeignKeys) {
						...
						table.ForeignKeys.Add(new DBForeignKey(cols, fk.PrimaryTable, rcols) {Name = fk.Name});
					}
				} catch {
				}
			}
	...
	public class DataAccessVistaDBProviderFactory : VistaDBProviderFactory {
		public override IDataStore CreateProviderFromConnection(IDbConnection connection, AutoCreateOption autoCreateOption) {
			return DataAccessVistaDBConnectionProvider.DataAccessCreateProviderFromConnection(connection, autoCreateOption);
		}
		public override IDataStore CreateProviderFromString(string connectionString, AutoCreateOption autoCreateOption, out IDisposable[] objectsToDisposeOnDisconnect) {
			return DataAccessVistaDBConnectionProvider.DataAccessCreateProviderFromString(connectionString, autoCreateOption, out objectsToDisposeOnDisconnect);
		}
	}
}
