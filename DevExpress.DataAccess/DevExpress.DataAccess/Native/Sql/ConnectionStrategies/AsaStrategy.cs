...
namespace DevExpress.DataAccess.Native.Sql.ConnectionStrategies {
	public class AsaStrategy : DbStrategyBase {
		DataAccessAsaProviderFactory factory;
		...
		public override ConnectionParameterEdits GetEditsSet(IConnectionParametersControl control) {
			const ConnectionParameterEdits editsServerBased = ConnectionParameterEdits.ServerName | ConnectionParameterEdits.Database | ConnectionParameterEdits.Hostname;
			return ConnectionParameterEdits.ServerType | ConnectionParameterEdits.UserName | ConnectionParameterEdits.Password
				   | (control.ServerBased ? editsServerBased : ConnectionParameterEdits.FileName);
		}
		public override ConnectionParameterEdits Subscriptions {
			get {
				return ConnectionParameterEdits.ServerType;
			}
		}
		protected override ProviderFactory Factory {
			get {
				return this.factory ?? (this.factory = new DataAccessAsaProviderFactory());
			}
		}
		...
		public override DataConnectionParametersBase GetConnectionParameters(IConnectionParametersControl control) {
			return control.ServerBased
				? string.IsNullOrWhiteSpace(control.Hostname)
					? new AsaConnectionParameters(control.ServerName, control.Database, control.UserName, control.Password)
					: new AsaConnectionParameters(control.ServerName, control.Hostname, control.Database, control.UserName, control.Password)
				: new AsaConnectionParameters(control.FileName, control.UserName, control.Password);
		}
		...
		public override void InitializeControl(IConnectionParametersControl control, DataConnectionParametersBase value) {
			AsaConnectionParameters p = (AsaConnectionParameters)value;
			switch(p.ConnectionType) {
				case AsaConnectionType.ServerBased:
					control.ServerBased = true;
					control.ServerName = p.ServerName;
					control.Database = p.DatabaseName;
					break;
				case AsaConnectionType.File:
		...
		public override int GetDefaultIndex(ConnectionParameterEdits edit) {
			return edit == ConnectionParameterEdits.ServerType ? 0 : -1;
		}
