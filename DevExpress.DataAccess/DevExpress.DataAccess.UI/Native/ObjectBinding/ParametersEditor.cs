...
namespace DevExpress.DataAccess.UI.Native.ObjectBinding {
	public class ParametersEditor : UITypeEditor {
		...
		public override object EditValue(ITypeDescriptorContext context, IServiceProvider provider, object value) {
			...
			DesignerTransaction transaction = designerHost.CreateTransaction("Edit Parameters");
			...
			if(ods.EditParameters(solutionTypesProvider, wizardRunnerContext, parameterService, repositoryItemsProvider)) {
				componentChangeService.OnComponentChanged(ods, null, null, null);
				transaction.Commit();
			}
			else
				transaction.Cancel();
			return ods.Parameters;
		}
		#endregion
	}
}
