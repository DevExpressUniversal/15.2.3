...
namespace DevExpress.DataAccess.UI.Native.ObjectBinding {
	public class DataSourceEditor : UITypeEditor {
		...
		public override object EditValue(ITypeDescriptorContext context, IServiceProvider provider, object value) {
			...
			DesignerTransaction transaction = designerHost.CreateTransaction("Edit DataSource");
			...
			if(ods.EditDataSource(solutionTypesProvider, wizardRunnerContext, parameterService, repositoryItemsProvider)) {
				componentChangeService.OnComponentChanged(ods, null, null, null);
				transaction.Commit();
			} else
				transaction.Cancel();
			return ods.DataSource;
		}
		#endregion
	}
}
