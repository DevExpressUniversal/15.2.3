...
var ASPxClientGridView = ASPx.CreateClass(ASPxClientGridBase, {
    ...
    constructor: function(name){
        ...
        this.isVirtualSmoothScrolling = false;
    ...
    GetAdaptiveHeaderContainer: function(visibleIndex) { 
        var headerPanel = this.GetAdaptiveHeaderPanel();
        if(!headerPanel) return null;

        var adaptiveHeader = this.GetChildElement(this.AdaptiveHeaderID + visibleIndex); 
        if(!adaptiveHeader) {
                adaptiveHeader = this.GetSampleAdaptiveHeader().cloneNode(true);
                adaptiveHeader.id = this.name + "_" + this.AdaptiveHeaderID + visibleIndex;
                ...
                headerPanel.appendChild(adaptiveHeader);
            }
    ...
    GetSampleAdaptiveHeader: function() { 
        return this.GetChildElement(this.AdaptiveHeaderID); 
    },
    ...
    Initialize: function() {
        ...
        resizingHelper && resizingHelper.ResetStretchedColumnWidth();
    ...
    UpdateIndentCellWidths: function() {
        if(this.indentColumnCount <= 0) 
            return;
        if(this.HasScrolling() || this.AllowResizing()) // TODO Move to TableHelper
    ...
    RequireChangeItemStyle: function(visibleIndex, itemStyle){
        if(!ASPxClientGridBase.prototype.RequireChangeItemStyle.call(this, visibleIndex, itemStyle))
            return false;
        return itemStyle != ASPxClientGridItemStyle.Selected || !this.IsGroupRow(visibleIndex); // do not select the group row
    },
    ...
    IsHeaderChild: function(source) {
        ...
        if(!mainElement) 
            return false;
        if(!ASPx.GetIsParent(mainElement, source))
            return false;
    ...
    CalculateAdaptivity: function(){
        if(this.adaptivityMode === 0) return;
        ...
        if(this.adaptivityMode === 1) {
            ...
            if(this.hideDataCellsWindowInnerWidth > 0 && window.innerWidth <= this.hideDataCellsWindowInnerWidth){
            ...
            this.ForEachCell(table, 
                function(cell) { 
                    if(ASPx.ElementContainsCssClass(cell, this.AdaptiveHiddenCssClass) || 
                        ...
                        ASPx.ElementContainsCssClass(cell.parentNode, this.GroupRowCssClass))
                        return;
    ...
    HideAdaptiveColumns: function(){
        for(var i = 0; i < this.adaptiveColumnsOrder.length - 1; i++){
            ...
            if(this.GetVisibleColumnIndices().indexOf(columnIndex) == -1) continue;
    ...
    MoveHeaderToAdaptiveHeaderPanel: function(headerCell){
        var container = this.GetAdaptiveHeaderContainer(this.getColumnIndex(headerCell.id));            
    ...
    GetAdaptiveDetailsFormLayoutElement: function(parentElement) {
        ...
        var formLayoutElements = ASPx.GetChildNodesByPartialClassName(parentElement,  ASPx.FormLayoutConsts.FORM_LAYOUT_PARTIAL_CLASS_NAME);
        return formLayoutElements && formLayoutElements.length ? formLayoutElements[0] : null;
    },
    ...
    CreateAdaptiveDetailsFormLayoutContent: function(parentElement, visibleIndex){
        ...
        for(var i = 0; i < this.GetColumnCount(); i++) {
            ...
            if(dataCell) {
                if(this.GetVisibleColumnIndices().indexOf(column.index) > -1) {
                    dataCell.innerHTML = "";
                    ASPx.MoveChildrenToElement(sampleDataCell, dataCell);
    ...
    GetAdaptiveTitle: function(columnIndex){
        var header = this.GetHeader(columnIndex);
        var textNode = ASPx.GetNodeByTagName(header, "TD", 0);
        return ASPx.GetInnerText(textNode) + ":";
    },
    ...
    SetVerticalScrollPosition: function(value) {
        if(this.isVirtualScrolling && !this.isVirtualSmoothScrolling)
            return;
        var scrollHelper = this.GetScrollHelper();
        if(scrollHelper)
...
/* TODO public ASPxClientGridViewContextMenuEventArgs(string objectType, int index, object htmlEvent, object menu, bool showBrowserMenu) {} */ 
