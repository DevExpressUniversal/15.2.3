...
var ASPxClientPopupControl = ASPx.CreateClass(ASPxClientControl, {
    constructor: function(name) {
        ...
        this.leadingAfterInitCall = ASPxClientControl.LeadingAfterInitCallConsts.Direct; // popup container must correct size before it's content
    ...
    // fix: ondragstart event is not valid forXHTML
    ...
    DoShowWindowModalElement: function(index) {
        ...
        if(modalElement) {
            ...
            //Q519268, T315573 - Set unscrollable to gray backgroung div in IE
    ...
    CreateLoadingDiv: function(parentElement, offsetElement, windowIndex) {
        if(typeof (windowIndex) != "undefined") { //B238149 
    ...
    SetClientWindowSizeCoreLite: function(index, width, height, isWindowCollapsed) {
        ...
        if(typeof (width) != "undefined") {
            ...
            if(actualWidth <= 0)
                actualWidth = ASPx.Browser.WebKitFamily ? 1 : 0; //T186035, T243760
    ...
    SetWindowScrollDivSize: function(scrollDiv, index, dimension) {
        ...
        if(dimension == 'width' || dimension == 'both') {
            //NOTE: we are not using ASPx.SetOffsetWidth here due to weird
    ...
    StartSlideAnimation: function(animationDivElement, index, horizontalPopupPosition, verticalPopupPosition) {
        ...
        if(neddToForceAnimation) //T181540
            yPos = 1;
...
var ASPxClientPopupControlCollection = ASPx.CreateClass(ASPxClientControlCollection, {
    ...
    ActivateWindowElement: function(element, evt) {
        var topWindow = this.GetTopWindow(),
            topZIndex = topWindow && this.WindowZIndexWasInitialized(topWindow.style.zIndex) ? parseInt(topWindow.style.zIndex) : popupControlZIndex;
    ...
    IsDisappearAllowedByMouseOut: function(evt) {
        return ASPx.Browser.Firefox ? !this.IsOverPopupWindow(evt) : true;  //B185576
    },
    ...
    CalculateDocumentDimensionsWithoutPinnedWindows: function(evt) {    //Q514388: evt is a optional parameter
        var popupsToRestoreVisible = [];
    ...
    CalculateDocumentDimensionsWithoutPinnedWindowsOldIE: function(onCalculateFinished, evt) {  //Q514388: evt is a optional parameter
        var popupsToRestoreVisible = [];
        var needToHideRestorePopupsThatOutFromViewPort = !!evt && ASPx.Evt.GetEventSource(evt) == document;
        if(needToHideRestorePopupsThatOutFromViewPort)
            popupsToRestoreVisible = this.HidePinnedPopupsThatOutFromViewPort();
...
ASPx.PWMDown = function(evt, name, index, isWindowContentDraggingAllowed) {
    ...
    if(isWindowContentDraggingAllowed) { // B150637
        ...
        if(typeof (ASPx.GetDropDownCollection) == "function")
            ASPx.GetDropDownCollection().OnDocumentMouseDown(evt); // B156698

        if(!pointOnScrollBar) {
            if(!ASPx.Browser.WebKitTouchUI && ASPx.Evt.GetEventSource(evt).tagName == "IMG") // Prevent image dragging
                ASPx.Evt.PreventEvent(evt);
        }
    }
}
...
function aspxPWMEMDown(evt) {
    ...
    if(modalDiv != null) // Prevent re-popup after ModalElement MouseDown + PopupElemen MouseUp
