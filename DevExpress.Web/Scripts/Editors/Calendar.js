...
var ASPxClientCalendar = ASPx.CreateClass(ASPxClientEdit, {
    ...
    Initialize: function() {
        ...
        if(this.accessibilityCompliant)
            this.accessibilityHelper = new AccessibilityCalendarHelper(this);
    },
    ...
    SetValue: function(date) {  
        if(date) {
			date = this.GetCorrectedDate(date);
    ...
    ApplySelectionByDiff: function(selection, save) {
        ...
        var selectionEdges = this.dateRangeMode ? ASPx.Data.ArrayGetIntegerEdgeValues(dates) : null;
        var oldSelectionEdges = this.dateRangeMode ? ASPx.Data.ArrayGetIntegerEdgeValues(oldDates) : null;
        var date;
        ...
        if(this.accessibilityCompliant)
            this.UpdateAccessibilityHelper(dates);
    },
    UpdateAccessibilityHelper: function(dates) {
        var datesLength = dates.length;
        var multiSelectText = ASPx.AccessibilitySR.CalendarMultiSelectText;
        var stringDate = (datesLength > 1 && ASPx.IsExists(multiSelectText)) ? multiSelectText : "";
        for(var i = 0; i < datesLength; i++) {
            stringDate += this.GetDateString(dates[i]);
            if(i !== datesLength - 1)
                stringDate += ", ";
        }         
        this.accessibilityHelper.SetValue(stringDate);
    },
    GetDateString: function(date) {
        var stringDate = date.toDateString();
        var shortDayName = stringDate.slice(0, 3);
        var fullDayName = this.GetFullDayName(shortDayName);
        var shortMonthName = stringDate.slice(4, 7);
        var fullMonthName = this.GetFullMonthName(shortMonthName);
        stringDate = stringDate.replace(shortDayName, fullDayName).replace(shortMonthName, fullMonthName);
        return stringDate;
    },
    GetFullDayName: function(shortDayName) {
        var index = ASPx.CultureInfo.abbrDayNames.indexOf(shortDayName);
        return index != -1 ? ASPx.CultureInfo.dayNames[index] : "";
    },
    GetFullMonthName: function(shortMonthName) {
        var index = ASPx.CultureInfo.abbrMonthNames.indexOf(shortMonthName);
        return index != -1 ? ASPx.CultureInfo.monthNames[index] : "";
    },
    ...
    UpdateStateObject: function() {
        var stateObject = { };
        var visibleDate = this.GetCorrectedDate(this.visibleDate);
    ...
    OnArrowUp: function(evt) {
        if(this.IsFastNavigationActive()) 
            ...
            if(!this.IsDateWithinBoundaries(newDate))
                return true;
    ...
    OnDayMouseOver: function (date) {
        if (!this.DaysSelectingOnMouseOver.IsEmpty()) {
            var args = new ASPxDaysSelectingOnMouseOverEventArgs(date);
            this.DaysSelectingOnMouseOver.FireEvent(this, args);
            if (args.cancel)
                return;
            this.selectionStartDate = args.selectionStartDate;
            date = args.overDate;
        }
        if (!this.enableMulti && this.enableSwipeGestures && !this.dateRangeMode)
            return;
        if (this.enableMulti || this.dateRangeMode) {
    ...
    RaiseVisibleDaysMouseOut: function() {
        if (!this.VisibleDaysMouseOut.IsEmpty())
            this.VisibleDaysMouseOut.FireEvent(this);
    },
    ...
    GetNearestDayForToday: function() {
        ...
        if(this.minDate && !this.IsDateInRange(todayDate))
            todayDate = ASPxClientCalendar.CloneDate(this.IsDateDisabled(this.minDate) ? this.GetNearestDayForDate(this.minDate, "left") : this.minDate);
        return todayDate;
    },
    ...
    NeedCalculateSelectionOnUpdating: function() {
        return this.dateRangeMode && this.selection.startDate && this.selection.endDate && !this.forceUpdatingOnMouseOver;
    },
    ...
    CanMouseDown: function(evt, date){
        if(!evt) return false;
        return ASPx.Evt.IsLeftButtonPressed(evt) && this.CheckDateEnabled(date);
    },
    CanMouseOver: function(date){
        if(this.forceMouseDown || this.isMouseDown || this.dateRangeMode)
            return this.CheckDateEnabled(date);
        return false;
    },
...
var AccessibilityCalendarHelper = ASPx.CreateClass(null, {
    SetValueTimeout: 500, // TODO check value
    constructor: function(calendar) {
        this.calendar = calendar;
        this.isFirstChild = true;
    },
    GetChild: function(suffix) {
        return this.calendar.GetChildElement("AcAs" + suffix);
    },
    SetValue: function(value) {
        this.isFirstChild = !this.isFirstChild;
        setTimeout(function() {
            var currentChild = this.GetChild(this.isFirstChild ? 0 : 1);
            ASPx.Attr.SetAttribute(currentChild, "aria-label", value);
            ASPx.Attr.SetAttribute(this.GetChild(!this.isFirstChild ? 0 : 1), "aria-label", "");
            ASPx.Attr.SetAttribute(this.calendar.GetAccessibilityServiceElement(), "aria-activedescendant", currentChild.id);
        }.aspxBind(this), this.SetValueTimeout);
    }
})

window.AccessibilityCalendarHelper = AccessibilityCalendarHelper;
            ...
            switch(e.type) {
                ...
                case "mouseover":
                    if(allowed && cal.CanMouseOver(date)) {
                        ...
                        else if (cal.isMouseDown || cal.dateRangeMode)
                            cal.OnDayMouseOver(date);
                    }
                    break;
                case "mouseup":
                    if(cal.isMouseDown) {
                        if(allowed && cal.CheckDateEnabled(date))
                ...
                case "mouseout":
                    if (cal.dateRangeMode) {
                        ...
                        if (ASPx.IsExists(overCellIndex)) {
                            ...
                            isAllowedOverDate = overDate && cal.IsDateInRange(overDate) && view.IsDateVisible(overDate) && !cal.IsDateDisabled(overDate);
                        }
...
var ASPxClientCalendarView = ASPx.CreateClass(null, {
    ...
    GetDayCellIndex: function (cell) {
        ...
        var maxIndex = 7 * calendarWeekCount - 1;
        do {
    ...
    GetVisibleRangeRestrictions: function() {
        return {
            ...
            end: this.GetDateByIndex(7 * calendarWeekCount - 1)
        };
    },
    ...
    EnsureSelection: function() {
        ...
        var selectedRangeRestrictions = this.calendar.dateRangeMode ? ASPx.Data.ArrayGetIntegerEdgeValues(selection.GetDates()) : null;
