...
// [Seleznyov] The selected field and the GetColumnText and GetColumnText methods defined in the ASPxClientListBoxItem.
// This is made to simplify the help
...
var ASPxClientListBoxBase = ASPx.CreateClass(ASPxClientListEdit, {
    ...
    SelectIndex: function (index){ // TODO try remove it
    ...
    FindItemInArray: function(array, item){
	    for(var i = array.length - 1; i >= 0; i--){
	        ...
	        // B139647 if EncodeHtml = false then the texts could be different (one with tags and another without).
...
var ASPxClientListBox = ASPx.CreateClass(ASPxClientListBoxBase, {
    ...
    Initialize: function() {   
        ...
        if(this.accessibilityCompliant)
            this.screenReaderHelper = new ScreenReaderListBoxHelper(this);
    },
    ...
    InitScrollPos: function(){ // Fix auto-scroll
    ...
    GetItemCount: function(){
        ...
        if(lbt)
            return (this.accessibilityCompliant && this.IsMultiColumn()) ? this.GetListTable().rows.length - 1 : this.GetListTable().rows.length;
        return 0;
    },
    ...
    AdjustControl: function (nestedCall) {
        if(this.IsAdjustmentRequired()) // Q444047
    ...
    AdjustControlCore: function(){
        ...
        if(!this.isComboBoxList && ASPx.Browser.IE) // FIX orevent to IE7 invisible scrollbar accounting
            this.CorrectWidth();
    },
    ...
    CorrectHeight: function(){
        if(ASPx.Browser.Firefox && this.heightCorrected) return; // Prevent FF flicks
    ...
    CorrectNonMultiColumnWidth: function(){
        ...
        if(this.width == ""){
            var listTable = this.GetListTable();
            var mainElement = this.GetMainElement();
            if(listTable.offsetWidth != 0 || !ASPx.Browser.NetscapeFamily){ // Mozilla: Prevent width collapse when item collection is empty
                divElement.style.width = (listTable.offsetWidth + this.GetVerticalScrollBarWidth()) + "px";
                if(ASPx.Browser.Firefox) // Prevent right border invisibility
    ...
    SetScrollTop: function(scrollTop){
        var scrollDiv = this.GetScrollDivElement();
        if(scrollDiv){ // Render may be removed when callback processing
    ...
    GetVerticalScrollBarWidthCore: function(divElement){
        ...
        if(ASPx.Browser.IE)
            return this.scrollDivBordersWidthWithScroll; // Prevent IE7 Border invisible
    ...
    SelectIndexSilent: function(index){ // TODO remove it
    ...
    UpdateHiddenInputs: function(){
        if(this.IsUpdateHiddenInputsLocked())
            return;
        if(this.MultiSelectionMode()){ // TODO remove it into to some helper
            var element = this.FindStateInputElement();
            if(ASPx.IsExistsElement(element)) {
                var sb = [ ];
                var serialiser = this.GetSerializingHelper();
    ...
    OnAccessArrowUp: function(evt){
        ...
        this.GetItemSelectionHelper().focusedIndex = index;
        return true;
    },
    ...
    SelectNeighbour: function (step) {
        ...
        if(itemCount > 0) {
            ...
            var pageDownSize = step == 1 ? this.scrollPageSize + 1 : this.scrollPageSize; //B155108
    ...
    GetItem: function(index){
        var listTable = this.GetListTable();
        var rowLength = (this.accessibilityCompliant && this.IsMultiColumn()) ? listTable.rows.length - 1 : listTable.rows.length;
    ...
    ChangeListTableEvents: function(listTable, method){
        if(this.isComboBoxList){
            ...
            if(ASPx.Browser.Firefox)// FIX prevent to wront continues selection by item click
    ...
    OnItemSelectionChanged: function(index, selected){
        ...
        this.CacheValue();
        this.screenReaderHelper && this.screenReaderHelper.ChangeSelectedIndex(index);
    },
...
var ASPxClientNativeListBox = ASPx.CreateClass(ASPxClientListBoxBase, {
    ...
    InsertItemInternal: function(index, text, value, imageUrl) {
        ...
        ASPx.Data.ArrayInsert(this.itemsValue, value, index);
        this.RegisterInsertedItem(index, text, value, imageUrl); // TODO make possible to remove
...
var ScreenReaderListBoxHelper = ASPx.CreateClass(null, {
    constructor: function(listBox) {
        this.listBox = listBox;
        this.mainElement = null;
        this.firstReadingColumnIndex = 0;
        this.selectedAssistIndex = -1;
        this.Initialize();
    },
    Initialize: function() {
        this.mainElement = this.listBox.GetAccessibilityServiceElement();

        if(this.listBox.CheckColumnSelectionMode())
            this.firstReadingColumnIndex = 1;

        this.setLabelElement();
        if(this.listBox.CheckColumnSelectionMode() && !this.listBox.IsNativeCheckBoxes()) {
            this.listBox.GotFocus.AddHandler(function() { 
                var lastIndex = this.listBox.GetItemCount() - 1;
                if(!this.listBox.GetInternalCheckBox(lastIndex).focused)
                  setTimeout(function() { 
                      if(!this.listBox.focused) return;
                      this.listBox.GetInternalCheckBox(0).mainElement.focus(); 
                      this.listBox.GetItemSelectionHelper().focusedIndex = 0;
                  }.aspxBind(this), 1000);
            }.aspxBind(this));
        }
        this.ChangeSelectedIndex(this.listBox.GetSelectedIndex());
    },
    setLabelElement: function() {
        var captionCell = this.listBox.GetCaptionCell();
        if(captionCell) {
            ASPx.Attr.SetAttribute(this.mainElement, "aria-labelledby", captionCell.id);
            var captionLabel = captionCell.childNodes[0];
            ASPx.Attr.RemoveAttribute(captionLabel, "for");
            ...
            }.aspxBind(this.listBox));
        }
    },
    getItemCellText: function(rowIndex, columnIndex) {
        var itemRowElement = this.listBox.GetItemRow(rowIndex);
        return itemRowElement ? ASPx.GetInnerText(itemRowElement.cells[this.firstReadingColumnIndex + columnIndex]) : "";
    },
    getHeaderCellCaptionText: function(index) {
        var headerDiv = this.listBox.GetHeaderDivElement();
        if(!headerDiv) return "";
    ...
    getAssistElement: function(index) {
        return ASPx.GetElementById(this.mainElement.id + index);
    },
    getAssistElementIndex: function(index) {
        return index == 0 ? 1 : 0;
    },
    ChangeSelectedIndex: function(index) {
        var stateSelected = index != -1 && this.listBox.GetItem(index).selected;
        var selectedIndex = this.listBox.GetSelectedIndex();
        if(!this.listBox.CheckColumnSelectionMode() && !stateSelected && selectedIndex != -1) return;
        var isDelayedSelection = index > -1 && this.listBox.CheckColumnSelectionMode() && ASPx.Browser.WebKitFamily;
        setTimeout(function(){ this.setSelectionAttributes(index, stateSelected) }.aspxBind(this), isDelayedSelection ? 1000 : 0);
    },
    setSelectionAttributes: function(index, stateSelected) {
        if(index == -1) {
            this.selectedAssistIndex = 0;
            index = 0;
        } else {
            this.selectedAssistIndex = this.getAssistElementIndex(this.selectedAssistIndex);
        }
        var assistElement = this.getAssistElement(this.selectedAssistIndex);
        var otherAssist = this.getAssistElement(this.getAssistElementIndex(this.selectedAssistIndex));

        var assistText = "";
        if(!this.listBox.IsMultiColumn()){
            assistText = this.listBox.GetItem(index).text;
        } else {
            var columnsCount = this.listBox.columnFieldNames.length;
            var format = ASPx.AccessibilitySR.TableItemFormatString ? ASPx.AccessibilitySR.TableItemFormatString : "{0} column {1} value, "; 
            for(var i = 0; i < columnsCount; i++)
                assistText += ASPx.Str.ApplyReplacement(format, [["{0}", this.getHeaderCellCaptionText(i)], ["{1}", this.getItemCellText(index, i)]]);
        }
        this.ChangeActivityAssistAttributes(assistElement, {"role": "option", "aria-posinset": index + 1, "aria-setsize": this.listBox.itemTotalCount, "aria-label": assistText, "aria-selected": stateSelected});
        this.ChangeActivityAssistAttributes(otherAssist, {"role": "", "aria-posinset": "", "aria-setsize": "", "aria-label": "", "aria-selected": ""});

        ASPx.Attr.SetAttribute(this.mainElement, "aria-activedescendant", assistElement.id);
    },
    ChangeActivityAssistAttributes: function(element, argumentsObject) {
        for(key in argumentsObject) {
            var value = argumentsObject[key];
            value === "" ? ASPx.Attr.RemoveAttribute(element, key) :  ASPx.Attr.SetAttribute(element, key, value);
        }
    }
});
...
var ASPxClientCheckListBase = ASPx.CreateClass(ASPxClientListEdit, {
    constructor: function(name) {
        ...
        this.internalButtonCollection = null; // TODO rename
		this.icbFocusedStyle = [];
        this.items = [];
	},
    Initialize: function() {
        ASPxClientListEdit.prototype.Initialize.call(this);
        this.UpdateInternalSelectedIndex(); //Q321147
    },
    ...
    CreateButtonInternalCollection: function() { // TODO rename
...
var ASPxClientRadioButtonList = ASPx.CreateClass(ASPxClientCheckListBase, {
    ...
    InlineInitialize: function() {
        ...
        this.SetSelectedIndex(this.GetSelectedIndex()); // B134441
...
var ASPxClientCheckBoxList = ASPx.CreateClass(ASPxClientCheckListBase, {
    ...
    SetSelectionDecoration: function(index, selected){ // TODO check if native
        if(this.CheckableElementsExist()){
    ...
    UpdateHiddenInputs: function(){
        if(this.IsUpdateHiddenInputsLocked()) // Changed
            return;
        var element = this.FindStateInputElement();
        if(ASPx.IsExistsElement(element)) { // Not changed
            var sb = [ ];
            var serialiser = this.GetSerializingHelper();
            var selectedIndices = this.GetSelectedIndices();
            for(var i = 0; i < selectedIndices.length; i++)
