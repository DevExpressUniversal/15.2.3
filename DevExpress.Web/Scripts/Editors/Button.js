...
var ASPxClientButton = ASPx.CreateClass(ASPxClientControl, {
    ...
    ChangeEnabledEventsAttributes: function(method) {
        ...
        if(this.allowFocus){
            ...
            if(ASPx.Browser.Firefox){
                var image = this.GetButtonImage();
                if(image)
                    method(image, "mousedown", this.onImageMoseDown); // Q588593
            }
        }
    },
    ...
    RequiredPreventDoublePostback: function(){
        return ASPx.Browser.Firefox && !this.isNative; // Q261965
    },
    OnFocus: function() {
        if(!this.allowFocus)
            return false;
    ...
    OnClick: function(evt) {
        ...
        if(params.processOnServer && isValid){
            ...
            var postponePostback = ASPx.Browser.AndroidMobilePlatform; // B221000
            if(requiredPreventDoublePostback || postponePostback)
                window.setTimeout(function() { _aspxBCallButtonMethod(this.name, "SendPostBack", "Click"); }.aspxBind(this), 0); //B149115
            else
    ...
    SetFocus: function(){
        ...
        var element = this.GetInternalButton();
        if(element) {
            var hiddenInternalButtonRequiresVisibilityToGetFocused = ASPx.Browser.WebKitFamily /* Q39454 */ && !this.isNative /* B38849 */ && !this.IsLink();
    ...
    SetText: function(text){
        ...
        if(textContainer){
            if(this.isNative)
                ...
                if(this.clientVisible && ASPx.Browser.IE && ASPx.Browser.Version >= 9) //Q420906
