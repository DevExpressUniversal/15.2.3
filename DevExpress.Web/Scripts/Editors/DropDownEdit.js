...
var ASPxClientDropDownEditBase = ASPx.CreateClass(ASPxClientButtonEditBase, {
    ...
    Initialize: function(){
        ...
        if(this.RefocusOnClickRequired()){ // Strange bug fix B39122
            ASPx.Evt.AttachEventToElement(this.GetMainElement(), "click", function(evt) {
                if(this.GetInputElement() && ASPx.Evt.GetEventSource(evt).id != this.GetInputElement().id) // B197186
    ...
    GetDropDownParents: function() {
        ...
        while(element != null){
            if(element.tagName == "BODY")
                break;
            if(ASPx.Attr.IsExistsAttribute(element, "id")) { // T211361
    ...
    ShowDropDownArea: function(isRaiseEvent, forceSize) {
        ...
        if(!this.droppedDown) // B212683
            this.SetLockListBoxClick();
		this.lockClosing = true; // B38675
		
        var pc = this.GetPopupControl();
        var element = this.GetMainElement();
        var pcwElement = pc.GetWindowElement(-1);
        
        if(!ASPx.GetElementDisplay(pcwElement)) {// Bug fix
            pcwElement.style.visibility = "hidden";
        }

        ASPx.SetElementDisplay(pcwElement, true);
        if(ASPx.Browser.Chrome && pcwElement.clientHeight <= 0) // T263230
        ...
        if(ASPx.Browser.Chrome && pcwElement.clientHeight <= 0) // T263230 it's not a typo; need to check after installation size
            this.RestorePopupInsideNonStaticPositionedContainer();
        pc.popupVerticalOffset = - ASPx.GetClientTop(element);
        this.RaiseDropDownEventRequired = isRaiseEvent;
        var needRefreshHorzScroll = ASPx.Browser.Chrome && document.body.scrollWidth > document.body.clientWidth && pc.getParentPopupControl(-1); //T153370
    ...
    OnDropDown: function(evt) { 
        ...
        return ASPx.Evt.CancelBubble(evt); // Makes the StateController to hover items without releasing DropDownButton
    },
...
//        ASPx.GetControlCollection().SubscribeObjectToBrowserWindowResize(this);
    },
