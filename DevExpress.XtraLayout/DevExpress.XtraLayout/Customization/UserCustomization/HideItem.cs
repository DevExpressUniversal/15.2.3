...
namespace DevExpress.XtraLayout.Customization {
	class HideItem : InteractionMethod {
		public HideItem(UserInteractionHelper owner)
			: base(owner) {
			EditorType = EditorTypes.Button;
			Image = UCIconsHelper.HideIcon;
			Text = "Hide item";
		}
		public override bool CanExecute() {
			...
			foreach(BaseLayoutItem item in selItems) {
				if(!item.AllowHide || item == Owner.GetOwner().RootGroup) {
					return false;
				}
			}
			return true;
