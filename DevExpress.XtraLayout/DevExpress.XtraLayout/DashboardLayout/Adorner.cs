...
namespace DevExpress.XtraDashboardLayout {
	public class Adorner {
		DashboardLayoutControl ownerControl;
		public Adorner(Control adornedControl) {
			adornedControlCore = adornedControl;
			ownerControl = adornedControl as DashboardLayoutControl;
			CreateAdornerWindowWindow();
		}
		internal AdornerLayeredWindow adornerWindow;
		protected virtual void CreateAdornerWindowWindow() {
			adornerWindow = new AdornerLayeredWindow(ownerControl);
		}
		...
		static Control FindParentControl(Control control) {
		...
		public bool NeedHideShowAdorner() {
			...
			if(!adornerWindow.IsCreated) return false;
		...
		Size adornerPadding = new Size(0, 0);
		public void Show() {
			EnsureInitialized();
			Point ownerLocation = AdornedControl.PointToScreen(Point.Empty);
			Control ownerForm = FindParentControl(AdornedControl);
	...
	public class AdornerLayeredWindow : DXLayeredWindowEx {
		DashboardLayoutControl ownerControl;
		public AdornerLayeredWindow(DashboardLayoutControl dlc) {
			ownerControl = dlc;
		}
		protected override void DrawCore(Utils.Drawing.GraphicsCache cache) {
			ownerControl.handler.Paint(cache.Graphics);
		}
	}
}
