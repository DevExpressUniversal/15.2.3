...
namespace DevExpress.Persistent.Base.Security {
	public class RoleImpl {
		public static ReadOnlyCollection<IPermission> GetRolePermissions(IList<IPersistentPermission> persistentPermissions) {
			...
			foreach(IPersistentPermission persistentPermission in persistentPermissions) {
				if(persistentPermission.Permission != null) {
					permissions.Add(persistentPermission.Permission);
				}
			}
		...
		public ReadOnlyCollection<IPermission> GetPermissions(IList<IPersistentPermission> persistentPermissions) {
			return GetRolePermissions(persistentPermissions);
		}
		public string Name {
			get { return name; }
			set { name = value; }
