...
namespace DevExpress.Xpf.Scheduler.Commands {
	public class XpfEditNormalAppointmentUICommand : EditNormalAppointmentUICommand {
		public XpfEditNormalAppointmentUICommand(ISchedulerCommandTarget scheduler)
			: base(scheduler) {
		}
		public override void UpdateUIState(ICommandUIState state) {
			base.UpdateUIState(state);
			Appointment apt = ObtainEditedAppointment();
			state.Visible = apt == null || CanExecute(apt);
		}
	}
}
