...
namespace DevExpress.Office.History {
	...
	public class HistoryTransaction : IDisposable {
		readonly DocumentHistory history;
		...
		void Dispose(bool disposing) {
			if (!disposed) {
				if (disposing) {
					bool topLevelTransaction = (history.TransactionLevel == 1);
					if (topLevelTransaction && SuppressRaiseOperationComplete)
						history.SuppressRaiseOperationComplete = true;
					history.EndTransaction();
					if (topLevelTransaction && SuppressRaiseOperationComplete)
						history.SuppressRaiseOperationComplete = false;
				}
			}
			disposed = true;
		}
