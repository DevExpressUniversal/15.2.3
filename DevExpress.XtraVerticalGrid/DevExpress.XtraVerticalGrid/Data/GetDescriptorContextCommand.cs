	...
	public class GetDescriptorContextCommand {
		...
		public DescriptorContext Execute(string propertyName) {
			if(this.contextCache == null)
				return CreateCachedDescriptorContext(propertyName, null, null);
		...
		DescriptorContext RequestDescriptorContext(string propertyName) {
			...
			if(PropertyHelper.IsRoot(propertyName))
				return CreateCachedDescriptorContext(PropertyHelper.RootPropertyName, DataSource, null);
			...
			return CreateCachedDescriptorContext(propertyName, components, propertyDescriptor);
		}
		...
		PropertyDescriptor GetNextPropertyDescriptor(string propertyName, object value, DescriptorContext parentContext) {
			if(IsMultiSource)
				return MultiObjectPropertyDescriptor.GetMultiProp(parentContext, (object[])value, propertyName, BrowsableAttributes);
			else {
				return parentContext.GetProperty(value, BrowsableAttributes, propertyName);
			}
		}
		DescriptorContext CreateCachedDescriptorContext(string fieldName, object obj, PropertyDescriptor pd) {
#if !Win
			...
			DescriptorContext parentContext = ContextCache[IsMultiSource, FieldNameHelper.GetParentFieldName(fieldName)];
