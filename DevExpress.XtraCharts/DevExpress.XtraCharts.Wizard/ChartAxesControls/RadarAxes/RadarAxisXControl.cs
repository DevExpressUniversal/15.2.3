...
namespace DevExpress.XtraCharts.Wizard.ChartAxesControls {
	internal partial class RadarAxisXControl : AxisControlBase {
		public override XtraTabControl TabControl { get { return xtraTabControl; } }
		public RadarAxisXControl() {
			InitializeComponent();
			InitializeTags();
		}
		protected override void Initialize(UserLookAndFeel lookAndFeel) {
			base.Initialize(lookAndFeel);
			RadarAxisX axis = Axis as RadarAxisX;
			if (axis != null) {
				if (!generalTabsControl.Initialize(axis, Chart))
					xtraTabControl.TabPages.Remove(tbGeneral);
				axisAppearanceControl.Initialize(axis);
				axisElementsControl.Initialize(axis);
				radarAxisXLabelControl.Initialize(lookAndFeel, axis, Chart);
