...
namespace DevExpress.XtraVerticalGrid.Data {
	class MultiObjectPropertyDescriptor : PropertyDescriptor {
		...
		public static PropertyDescriptor GetMultiProp(DescriptorContext context, object[] components, string name, Attribute[] attrs, bool onlyBrowsable) {
			PropertyDescriptor[] props = new PropertyDescriptor[components.Length];
			PropertyDescriptor firstDescriptor = context.GetProperty(components[0], attrs, name, onlyBrowsable);
			...
			for(int i = 0; i < components.Length; i++) {
				if(i == 0) {
					...
					PropertyDescriptor othersDescriptor = context.GetProperty(components[i], attrs, name, onlyBrowsable);
