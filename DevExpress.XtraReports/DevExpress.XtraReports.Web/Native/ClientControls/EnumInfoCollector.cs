...
namespace DevExpress.XtraReports.Web.Native.ClientControls {
	public static class EnumInfoCollector {
		public static EnumInfo[] Collect(IEnumerable<ParameterPath> parameters) {
			var types = parameters
				.Select(x => x.Parameter.Type)
				.Where(x => x.IsEnum);
			return EnumInfoProvider.GetEnumInfo(types);
		}
	}
}
