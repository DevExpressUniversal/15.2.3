...
namespace DevExpress.XtraReports.Web.Native.DocumentViewer {
	class WebRemoteDocument : PrintingDocument {
		public WebRemoteDocument(PrintingSystemBase ps, int pageCount)
			: base(ps, null, null) {
			var pageFactory = new EmptyPageFactory();
			for(int i = 0; i < pageCount; i++) {
				var page = pageFactory.CreateEmptyPage(i, pageCount);
				Pages.AddPageInternal(page);
			}
		}
