...
namespace DevExpress.ReportServer.ServiceModel.ConnectionProviders {
	public class WindowsUserConnectionProvider : ConnectionProvider {
		...
		public override Task<bool> LoginAsync() {
			return LoginCoreAsync(null, null);
		}
		protected override AuthenticationServiceClientFactory CreateAuthClientFactory() {
			AuthenticationServiceClientFactory factory = new AuthenticationServiceClientFactory(GetEndpointAddress("WindowsAuthentication/AuthenticationService.svc"), AuthenticationType.Windows);
			factory.ChannelFactory.Endpoint.Behaviors.Add(CookieBehavior);
			return factory;
		}
	}
}
