#region Copyright (c) 2000-2015 Developer Express Inc.
/*
{*******************************************************************}
{                                                                   }
{       Developer Express .NET Component Library                    }
{                                                                   }
{                                                                   }
{       Copyright (c) 2000-2015 Developer Express Inc.              }
{       ALL RIGHTS RESERVED                                         }
{                                                                   }
{   The entire contents of this file is protected by U.S. and       }
{   International Copyright Laws. Unauthorized reproduction,        }
{   reverse-engineering, and distribution of all or any portion of  }
{   the code contained in this file is strictly prohibited and may  }
{   result in severe civil and criminal penalties and will be       }
{   prosecuted to the maximum extent possible under the law.        }
{                                                                   }
{   RESTRICTIONS                                                    }
{                                                                   }
{   THIS SOURCE CODE AND ALL RESULTING INTERMEDIATE FILES           }
{   ARE CONFIDENTIAL AND PROPRIETARY TRADE                          }
{   SECRETS OF DEVELOPER EXPRESS INC. THE REGISTERED DEVELOPER IS   }
{   LICENSED TO DISTRIBUTE THE PRODUCT AND ALL ACCOMPANYING .NET    }
{   CONTROLS AS PART OF AN EXECUTABLE PROGRAM ONLY.                 }
{                                                                   }
{   THE SOURCE CODE CONTAINED WITHIN THIS FILE AND ALL RELATED      }
{   FILES OR ANY PORTION OF ITS CONTENTS SHALL AT NO TIME BE        }
{   COPIED, TRANSFERRED, SOLD, DISTRIBUTED, OR OTHERWISE MADE       }
{   AVAILABLE TO OTHER INDIVIDUALS WITHOUT EXPRESS WRITTEN CONSENT  }
{   AND PERMISSION FROM DEVELOPER EXPRESS INC.                      }
{                                                                   }
{   CONSULT THE END USER LICENSE AGREEMENT FOR INFORMATION ON       }
{   ADDITIONAL RESTRICTIONS.                                        }
{                                                                   }
{*******************************************************************}
*/
#endregion Copyright (c) 2000-2015 Developer Express Inc.

using System;
using System.Collections.Generic;
using System.Text;
using System.ComponentModel;
using System.Collections;
using DevExpress.Utils.Design;
#if SILVERLIGHT
using DevExpress.Data.Browsing;
using TypeConverter = DevExpress.Data.Browsing.TypeConverter;
using DevExpress.Xpf.ComponentModel;
#endif
namespace DevExpress.XtraPrinting.Native.ExportOtions {
	public class ExportModeTypeProvider : TypeDescriptionProvider {
		object[] modes;
		Type exportModeType;
		public ExportModeTypeProvider(object[] modes, Type exportModeType) {
			if(modes.Length == 0)
				throw new ArgumentException("modes");
			if(exportModeType == null)
				throw new ArgumentNullException("exportModeType");
			this.modes = modes;
			this.exportModeType = exportModeType;
		}
		public object Validate(object value) {
			return Array.IndexOf(modes, value) >= 0 ? value : modes[0];
		}
		public override ICustomTypeDescriptor GetTypeDescriptor(Type objectType, object instance) {
			ICustomTypeDescriptor customTypeDescriptor = base.GetTypeDescriptor(objectType, instance);
			if(objectType == exportModeType)
				return new CustomEnumDescriptor(customTypeDescriptor, new CustomEnumTypeConverter(modes, exportModeType));
			return customTypeDescriptor;
		}
	}
	public class CustomEnumDescriptor : CustomTypeDescriptor {
		TypeConverter typeConverter;
		public CustomEnumDescriptor(ICustomTypeDescriptor customTypeDescriptor, TypeConverter typeConverter)
			: base(customTypeDescriptor) {
			this.typeConverter = typeConverter;
		}
		public override TypeConverter GetConverter() {
			return typeConverter;
		}
	}
	public class CustomEnumTypeConverter : EnumTypeConverter {
		object[] modes;
		public CustomEnumTypeConverter(object[] modes, Type exportModeType)
			: base(exportModeType) {
			this.modes = modes;
		}
		public override StandardValuesCollection GetStandardValues(ITypeDescriptorContext context) {
			return new StandardValuesCollection(modes);
		}
	} 
}
