#region Copyright (c) 2000-2015 Developer Express Inc.
/*
{*******************************************************************}
{                                                                   }
{       Developer Express .NET Component Library                    }
{                                                                   }
{                                                                   }
{       Copyright (c) 2000-2015 Developer Express Inc.              }
{       ALL RIGHTS RESERVED                                         }
{                                                                   }
{   The entire contents of this file is protected by U.S. and       }
{   International Copyright Laws. Unauthorized reproduction,        }
{   reverse-engineering, and distribution of all or any portion of  }
{   the code contained in this file is strictly prohibited and may  }
{   result in severe civil and criminal penalties and will be       }
{   prosecuted to the maximum extent possible under the law.        }
{                                                                   }
{   RESTRICTIONS                                                    }
{                                                                   }
{   THIS SOURCE CODE AND ALL RESULTING INTERMEDIATE FILES           }
{   ARE CONFIDENTIAL AND PROPRIETARY TRADE                          }
{   SECRETS OF DEVELOPER EXPRESS INC. THE REGISTERED DEVELOPER IS   }
{   LICENSED TO DISTRIBUTE THE PRODUCT AND ALL ACCOMPANYING .NET    }
{   CONTROLS AS PART OF AN EXECUTABLE PROGRAM ONLY.                 }
{                                                                   }
{   THE SOURCE CODE CONTAINED WITHIN THIS FILE AND ALL RELATED      }
{   FILES OR ANY PORTION OF ITS CONTENTS SHALL AT NO TIME BE        }
{   COPIED, TRANSFERRED, SOLD, DISTRIBUTED, OR OTHERWISE MADE       }
{   AVAILABLE TO OTHER INDIVIDUALS WITHOUT EXPRESS WRITTEN CONSENT  }
{   AND PERMISSION FROM DEVELOPER EXPRESS INC.                      }
{                                                                   }
{   CONSULT THE END USER LICENSE AGREEMENT FOR INFORMATION ON       }
{   ADDITIONAL RESTRICTIONS.                                        }
{                                                                   }
{*******************************************************************}
*/
#endregion Copyright (c) 2000-2015 Developer Express Inc.

using System;
using System.Collections;
using System.ComponentModel;
using System.Reflection;
using System.Security.Permissions;
using DevExpress.XtraPrinting;
using DevExpress.XtraPrinting.Localization;
namespace DevExpress.XtraReports.Native {
	public class XRAccessor : Accessor {
		XRAccessor() {
		}
		public static IList GetFieldValues(object obj, Type fieldType) {
			FieldInfo[] fieldInfos = GetFields(obj, fieldType, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.DeclaredOnly);
			ArrayList values = new ArrayList();
			foreach(FieldInfo field in fieldInfos)
				values.Add(field.GetValue(obj));
			return values;
		}
		public static FieldInfo[] GetFields(object obj, Type fieldType) {
			return GetFields(obj, fieldType, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
		}
		static FieldInfo[] GetFields(object obj, Type fieldType, BindingFlags bindingAttr) {
			FieldInfo[] fieldInfos = obj.GetType().GetFields(bindingAttr);
			ArrayList fields = new ArrayList();
			foreach(FieldInfo field in fieldInfos)
				if(fieldType.IsAssignableFrom(field.FieldType))
					fields.Add(field);
			return (FieldInfo[])fields.ToArray(typeof(FieldInfo));
		}
		public static PropertyDescriptor GetFastPropertyDescriptor(PropertyDescriptor descriptor) {
			if(!DevExpress.Data.Helpers.SecurityHelper.IsPermissionGranted(new ReflectionPermission(ReflectionPermissionFlag.MemberAccess)) ||
				!DevExpress.Data.Helpers.SecurityHelper.IsPermissionGranted(new SecurityPermission(SecurityPermissionFlag.ControlEvidence)))
				return descriptor;
			try {
				return DevExpress.Data.Access.DataListDescriptor.GetFastProperty(descriptor);
			} catch(System.Security.SecurityException) {
				return descriptor;
			}
		}
		public static PropertyDescriptor GetPropertyDescriptor(object obj, string propName) {
			PropertyDescriptor descriptor = TypeDescriptor.GetProperties(obj)[propName];
			if(descriptor == null) {
				throw (new ArgumentException(PreviewLocalizer.GetString(PreviewStringId.Msg_InvPropName), propName));
			}
			return descriptor;
		}
		public static void ChangeProperty(object obj, string propName, object value) {
			PropertyDescriptor descriptor = GetPropertyDescriptor(obj, propName);
			if(descriptor != null) {
				object oldValue = descriptor.GetValue(obj);
				if(oldValue == value)
					return;
				descriptor.SetValue(obj, value);
			}
		}
		public static Type GetPropertyType(object obj, string propName) {
			PropertyDescriptor descriptor = GetPropertyDescriptor(obj, propName);
			return descriptor != null ? descriptor.PropertyType : null;
		}
	}
	public static class PropInfoAccessorBase {
		public static PropertyInfo GetProperty(object obj, string propName, bool throwOnError) {
			PropertyInfo property = obj.GetType().GetProperty(propName);
			if(property == null && throwOnError)
				throw (new ArgumentException(PreviewLocalizer.GetString(PreviewStringId.Msg_InvPropName), propName));
			return property;
		}
		public static PropertyInfo GetProperty(object obj, string propName, Type propType) {
			PropertyInfo[] properties = obj.GetType().GetProperties();
			PropertyInfo result = null;
			foreach(PropertyInfo property in properties) {
				if(property.Name == propName && property.PropertyType == propType) {
					result = property;
					break;
				}
			}
			if(result == null)
				throw new ArgumentException(PreviewLocalizer.GetString(PreviewStringId.Msg_InvPropName), propName);
			return result;
		}
	}
}
