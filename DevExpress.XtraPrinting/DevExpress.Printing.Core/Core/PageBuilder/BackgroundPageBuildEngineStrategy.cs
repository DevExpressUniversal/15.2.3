...
namespace DevExpress.XtraPrinting.Native {
	public abstract class BackgroundPageBuildEngineStrategy {
		readonly static Pair<int, int>[] bufferSizes = new Pair<int, int>[] {
			new Pair<int, int>(1, 1), 
			new Pair<int, int>(5, 2), 
			new Pair<int, int>(15, 5), 
			new Pair<int, int>(35, 10), 
			new Pair<int, int>(75, 20), 
		};
		...
		public virtual int GetBufferSize(int count) {
			foreach(Pair<int, int> item in bufferSizes)
				if(count < item.First)
					return item.Second;
			return 50;
		}
