...
namespace DevExpress.XtraPrinting.XamlExport {
	class BrickContainerXamlExporter : BrickXamlExporterBase {
		public override void WriteBrickToXaml(XamlWriter writer, BrickBase brick, XamlExportContext exportContext, RectangleF clipRect, Action<XamlWriter> declareNamespaces, Action<XamlWriter, object> writeCustomProperties) {
			...
			if(!brick.NoClip) {
				...
				var brickContainer = (BrickContainer)brick;
				var clipRectInPixels = new RectangleF(brickContainer.Brick.X, -brickContainer.InnerBrickListOffset.Y, brickContainer.Width, brickContainer.Height).DocToDip();
