...
namespace DevExpress.XtraRichEdit.Import.Rtf {
	#region ShapePropertyPIBDestination
	public class ShapePropertyPIBDestination : DestinationBase {
		...
		RtfImageInfo imageInfo;
		public RtfImageInfo ImageInfo { get { return imageInfo; } }
		public ShapePropertyPIBDestination(RtfImporter rtfImporter)
			: base(rtfImporter) {
		}
		...
		protected override bool ProcessKeywordCore(string keyword, int parameterValue, bool hasParameter) {
			if (keyword == "pict") {
				PictureDestination destination = new PictureDestination(Importer);
		...
		protected override DestinationBase CreateClone() {
			ShapePropertyPIBDestination clone = new ShapePropertyPIBDestination(Importer);
			clone.imageInfo = imageInfo;
			return clone;
		}
		public override void NestedGroupFinished(DestinationBase nestedDestination) {
			...
			if (pictDest != null)
				imageInfo = pictDest.GetImageInfo();
		}
	}
	#endregion
}
