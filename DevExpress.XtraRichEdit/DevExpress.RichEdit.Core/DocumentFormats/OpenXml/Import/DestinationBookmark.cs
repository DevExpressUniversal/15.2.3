#region Copyright (c) 2000-2015 Developer Express Inc.
/*
{*******************************************************************}
{                                                                   }
{       Developer Express .NET Component Library                    }
{                                                                   }
{                                                                   }
{       Copyright (c) 2000-2015 Developer Express Inc.              }
{       ALL RIGHTS RESERVED                                         }
{                                                                   }
{   The entire contents of this file is protected by U.S. and       }
{   International Copyright Laws. Unauthorized reproduction,        }
{   reverse-engineering, and distribution of all or any portion of  }
{   the code contained in this file is strictly prohibited and may  }
{   result in severe civil and criminal penalties and will be       }
{   prosecuted to the maximum extent possible under the law.        }
{                                                                   }
{   RESTRICTIONS                                                    }
{                                                                   }
{   THIS SOURCE CODE AND ALL RESULTING INTERMEDIATE FILES           }
{   ARE CONFIDENTIAL AND PROPRIETARY TRADE                          }
{   SECRETS OF DEVELOPER EXPRESS INC. THE REGISTERED DEVELOPER IS   }
{   LICENSED TO DISTRIBUTE THE PRODUCT AND ALL ACCOMPANYING .NET    }
{   CONTROLS AS PART OF AN EXECUTABLE PROGRAM ONLY.                 }
{                                                                   }
{   THE SOURCE CODE CONTAINED WITHIN THIS FILE AND ALL RELATED      }
{   FILES OR ANY PORTION OF ITS CONTENTS SHALL AT NO TIME BE        }
{   COPIED, TRANSFERRED, SOLD, DISTRIBUTED, OR OTHERWISE MADE       }
{   AVAILABLE TO OTHER INDIVIDUALS WITHOUT EXPRESS WRITTEN CONSENT  }
{   AND PERMISSION FROM DEVELOPER EXPRESS INC.                      }
{                                                                   }
{   CONSULT THE END USER LICENSE AGREEMENT FOR INFORMATION ON       }
{   ADDITIONAL RESTRICTIONS.                                        }
{                                                                   }
{*******************************************************************}
*/
#endregion Copyright (c) 2000-2015 Developer Express Inc.

using System;
using System.Xml;
using DevExpress.XtraRichEdit.Model;
using DevExpress.XtraRichEdit.Internal;
namespace DevExpress.XtraRichEdit.Import.OpenXml {
	#region BookmarkElementDestination (abstract class)
	public abstract class BookmarkElementDestination : LeafElementDestination {
		protected BookmarkElementDestination(WordProcessingMLBaseImporter importer)
			: base(importer) {
		}
		public override void ProcessElementOpen(XmlReader reader) {
			string id = reader.GetAttribute("id", Importer.WordProcessingNamespaceConst);
			if (id != null)
				id = id.Trim();
			if (String.IsNullOrEmpty(id))
				return;
			ImportBookmarkInfo bookmark;
			if (!Importer.Bookmarks.TryGetValue(id, out bookmark)) {
				bookmark = new ImportBookmarkInfo();
				Importer.Bookmarks.Add(id, bookmark);
			}
			string name = reader.GetAttribute("name", Importer.WordProcessingNamespaceConst);
			if (name != null) {
				name = name.Trim();
				bookmark.Name = name;
			}
			AssignBookmarkPosition(bookmark);
		}
		protected internal abstract void AssignBookmarkPosition(ImportBookmarkInfo bookmark);
	}
	#endregion
	#region BookmarkStartElementDestination
	public class BookmarkStartElementDestination : BookmarkElementDestination {
		public BookmarkStartElementDestination(WordProcessingMLBaseImporter importer)
			: base(importer) {
		}
		protected internal override void AssignBookmarkPosition(ImportBookmarkInfo bookmark) {
			bookmark.Start = Importer.Position.LogPosition;
		}
	}
	#endregion
	#region BookmarkEndElementDestination
	public class BookmarkEndElementDestination : BookmarkElementDestination {
		public BookmarkEndElementDestination(WordProcessingMLBaseImporter importer)
			: base(importer) {
		}
		protected internal override void AssignBookmarkPosition(ImportBookmarkInfo bookmark) {
			bookmark.End = Importer.Position.LogPosition;
		}
	}
	#endregion
}
