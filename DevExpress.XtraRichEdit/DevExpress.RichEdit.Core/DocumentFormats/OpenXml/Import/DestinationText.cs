...
namespace DevExpress.XtraRichEdit.Import.OpenXml {
	...
	public class TextDestination : ElementDestination {
		...
		public TextDestination(WordProcessingMLBaseImporter importer)
			: base(importer) {
		}
		...
		public override bool ProcessText(XmlReader reader) {
			...
			if (reader.XmlSpace != XmlSpace.Preserve)
				text = RemoveRedundantSpaces(text);
			if (!String.IsNullOrEmpty(text)) {
				string plainText = StringHelper.ReplaceParagraphMarksWithLineBreaks(text);
				if(!String.IsNullOrEmpty(plainText))
					Importer.PieceTable.InsertTextCore(Importer.Position, plainText);				
			}
			return true;
		}	   
