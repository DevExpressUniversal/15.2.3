...
namespace DevExpress.XtraRichEdit.Import.WordML {
	...
	public abstract class SectionDestination : DevExpress.XtraRichEdit.Import.OpenXml.SectionDestinationBase {
		...
		protected static ElementHandlerTable CreateElementHandlerTable() {
			...
			result.Add("hdr", OnSectionHeader);
			result.Add("ftr", OnSectionFooter);
		...
		static Destination OnSectionHeader(WordProcessingMLBaseImporter importer, XmlReader reader) {
			if (importer.DocumentModel.DocumentCapabilities.HeadersFootersAllowed)
				return new DevExpress.XtraRichEdit.Import.WordML.HeaderDestination(importer);
			return new EmptyDestination(importer);
		}
		static Destination OnSectionFooter(WordProcessingMLBaseImporter importer, XmlReader reader) {
			if (importer.DocumentModel.DocumentCapabilities.HeadersFootersAllowed)
				return new DevExpress.XtraRichEdit.Import.WordML.FooterDestination(importer);
			return new EmptyDestination(importer);
		}
	}
	#endregion
