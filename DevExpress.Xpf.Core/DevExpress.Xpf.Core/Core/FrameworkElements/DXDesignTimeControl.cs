...
namespace DevExpress.Xpf.Core {
	public abstract class DXDesignTimeControl : Control {
		...
		[DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden), EditorBrowsable(EditorBrowsableState.Never), Browsable(false)]
		public new KeyboardNavigationMode TabNavigation {
			get { return base.TabNavigation; }
			set { base.TabNavigation = value; }
		}
#else // WPF
		[Browsable(false), EditorBrowsable(EditorBrowsableState.Never), DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
...
#pragma warning restore 0618
		...
		public new event MouseButtonEventHandler PreviewMouseDoubleClick {
			add { base.PreviewMouseDoubleClick += value; }
			remove { base.PreviewMouseDoubleClick -= value; }
		}
		[Browsable(false), EditorBrowsable(EditorBrowsableState.Never), DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		...
		protected virtual ControlTemplate CreateControlTemplate() {
			string template = "<Image x:Name=\"PART_Icon\" Stretch=\"None\">";
#if !SL
			template += "<RenderOptions.BitmapScalingMode>NearestNeighbor</RenderOptions.BitmapScalingMode>";
#endif
			template += "</Image>";
		...
		void DXDesignTimeControl_Loaded(object sender, RoutedEventArgs e) {
			ResetSize();
			SetTemplate();
		}
		...
		public override void OnApplyTemplate() {
			...
			if(!DesignerProperties.GetIsInDesignMode(this) || Template == null) return;
			using(System.IO.Stream stream = GetType().Assembly.GetManifestResourceStream(GetDesignTimeImageName())) {
				Image icon = (Image)GetTemplateChild("PART_Icon");
				icon.Source = (ImageSource)ImageDataConverter.CreateImageFromStream(stream);
			}
		}
		public DXDesignTimeControl() {
			Loaded += new RoutedEventHandler(DXDesignTimeControl_Loaded);
		}
	}
}
