...
namespace DevExpress.Xpf.Utils.Native {
	public static class ValidateFrameworkVersionHelper {
		static class WinFormsMessageBoxHelper {
		...
		public static bool Perform(string errorMessage) {
			Version minimalVersion = new Version(3, 5, 30729, 1);
			Version version = Helpers.GetFrameworkVersion();
			if (version == null || version < minimalVersion) {
				if(!string.IsNullOrEmpty(errorMessage)) {
				string message = string.Format(errorMessage, minimalVersion);
				WinFormsMessageBoxHelper.Show(message);
				}
				return false;
			}
			return true;
