...
namespace DevExpress.Xpf.Editors.Internal {
	public class ColorPickerColorModeTemplateSelector : DataTemplateSelector {
		public override DataTemplate SelectTemplate(object item, DependencyObject container) {
			...
			var model = item as ColorBase;
			if (element == null || model == null)
				return base.SelectTemplate(item, container);
			DataTemplate dataTemplate;
			string key = model.ColorMode + "DataTemplate";
			dataTemplate = (DataTemplate)element.FindResource(new ColorPickerThemeKeyExtension() { ResourceKey = (ColorPickerThemeKeys)Enum.Parse(typeof(ColorPickerThemeKeys), key, false) });
			return dataTemplate;
		}
	}
}
