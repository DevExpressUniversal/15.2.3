...
namespace DevExpress.Xpf.Editors.Internal {
	public class BarSparklineMapping : SparklineMappingBase {
		public BarSparklineMapping(IList<SparklinePoint> sortedPoints, Bounds bounds, InternalRange argumentRange, InternalRange valueRange)
			: base(sortedPoints, bounds, argumentRange, valueRange) {
		}
		protected override void CorrectValueRange(InternalRange valueRange) {
			if (valueRange.Max >= 0 && valueRange.Min >= 0)
				valueRange.CorrectionMin = (valueRange.Max - valueRange.Min) * 0.1;
			else if (valueRange.Max <= 0 && valueRange.Min <= 0)
