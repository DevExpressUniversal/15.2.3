...
namespace DevExpress.Xpf.Editors {
	public class CustomDialogServiceEditorBehavior : DialogServiceEditorBehaviorBase {
		protected override void ProcessClickForFrameworkElementInternal(FrameworkElement owner, object editValue) {
			var dialogService = DialogServiceTemplate.With(TemplateHelper.LoadFromTemplate<ICustomDialogService>);
			if (dialogService != null) {
				AssignableServiceHelper2<DependencyObject, ICustomDialogService>.DoServiceAction(owner, dialogService, service => {
					var value = GetEditableValue(owner, editValue);
					RaiseDialogShown(value);
					var dialogResult = service.ShowDialog(Title, value);
