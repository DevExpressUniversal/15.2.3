...
namespace DevExpress.Xpf.Editors.Helpers {
	public abstract class PlainDataView : DataView {
		new IList ListSource { get { return base.ListSource as IList; } }
		protected PlainDataView(object view, string valueMember, string displayMember)
			: base(view, valueMember, displayMember) {
		}
		public override bool ProcessAddItem(int index) {
			DataProxy proxy = DataAccessor.CreateProxy(ListSource[index], -1);
		...
		protected override void InitializeView(object source) {
			var view = (DefaultDataView)source;
			SetView(new LocalDataProxyViewCache(DataAccessor, view.Select(x => DataAccessor.CreateProxy(x, -1))));
		}
	}
}
