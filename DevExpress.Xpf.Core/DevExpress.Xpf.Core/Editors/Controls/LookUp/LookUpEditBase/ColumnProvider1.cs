...
namespace DevExpress.Xpf.Editors {
	public class SearchControlColumnProvider : Decorator, IFilteredComponent, ISearchPanelColumnProvider {
		...
		static SearchControlColumnProvider() {
			...
			AdditionalPropertiesProperty = DependencyProperty.Register("AdditionalProperties", typeof(Filtering.PropertyInfoCollection), typeof(SearchControlColumnProvider), new PropertyMetadata(null, Update));
			HiddenPropertiesProperty = DependencyProperty.Register("HiddenProperties", typeof(PropertyNameCollection), typeof(SearchControlColumnProvider), new PropertyMetadata(null, Update));
			UpperCasePropertyNamesProperty = DependencyProperty.Register("UpperCasePropertyNames", typeof(bool), typeof(SearchControlColumnProvider), new PropertyMetadata(false, Update));
		...
		void OnSourceControlChanged(DependencyPropertyChangedEventArgs e) {
			(e.OldValue as IFilteredComponent).Do(x => x.PropertiesChanged -= SearchControlColumnProvider_PropertiesChanged);
			(e.OldValue as IFilteredComponent).Do(x => x.RowFilterChanged -= SearchControlColumnProvider_RowFilterChanged);
			(e.NewValue as IFilteredComponent).Do(x => x.PropertiesChanged += SearchControlColumnProvider_PropertiesChanged);
			(e.NewValue as IFilteredComponent).Do(x => x.RowFilterChanged += SearchControlColumnProvider_RowFilterChanged);
			Update();
		}
		void SearchControlColumnProvider_PropertiesChanged(object sender, EventArgs e) {
			filteredComponentPropertiesChanged.Do(x => x(this, EventArgs.Empty));
		}
		void SearchControlColumnProvider_RowFilterChanged(object sender, EventArgs e) {
		...
		#endregion
		#region ISearchPanelColumnProvider Members
		IEnumerable<string> ISearchPanelColumnProvider.Columns {
			get { return GetAllColumns().Select(x => x.FieldName); }
		}
		...
		bool ISearchPanelColumnProviderBase.UpdateFilter(string searchText, FilterCondition filterCondition, CriteriaOperator filterCriteria) {
			SourceControl.RowCriteria = filterCriteria;
			return true;
		}
		System.Collections.ObjectModel.ObservableCollection<string> ISearchPanelColumnProviderBase.CustomColumns {
			get {
				throw new NotImplementedException();
			}
			set {
