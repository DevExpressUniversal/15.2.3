	...
	public class DateTimePickerDataCaptionConverter : IValueConverter {
		public object Convert(object value, Type targetType, object parameter, System.Globalization.CultureInfo culture) {
			if (!(value is DateTimePickerData))
				return string.Empty;
			DateTimePickerData pickerData = (DateTimePickerData)value;
			switch(pickerData.DateTimePart) {
				case DateTimePart.Day:
					return string.Format("{0:dddd}", pickerData.Value).ToLowerInvariant();
				...
				case DateTimePart.Month:
					return string.Format("{0:MMMM}", pickerData.Value).ToLowerInvariant();
