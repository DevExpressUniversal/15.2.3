...
namespace DevExpress.Xpf.Core.ServerMode {
	...
	public class LinqInstantFeedbackDataSource : ListSourceDataSourceBase, IQueryableServerModeDataSource, IDisposable {
		...
		static void OnAreSourceRowsThreadSafeChanged(DependencyObject d, DependencyPropertyChangedEventArgs e) {
			LinqInstantFeedbackSource data = ((LinqInstantFeedbackDataSource)d).Data as LinqInstantFeedbackSource;
			if(data != null) {
		...
		static void OnQueryableSourceChanged(DependencyObject d, DependencyPropertyChangedEventArgs e) {
			LinqInstantFeedbackDataSource dataSource = (LinqInstantFeedbackDataSource)d;
			dataSource.queryableSourceField = dataSource.QueryableSource;
			dataSource.ResetLinqInstantFeedbackSource();
		}
		static LinqInstantFeedbackDataSource() {
			Type ownerType = typeof(LinqInstantFeedbackDataSource);
		...
		void Data_DismissQueryable(object sender, DevExpress.Data.Linq.GetQueryableEventArgs e) {
			if(isDisposed) {
				LinqInstantFeedbackSource linqSource = (LinqInstantFeedbackSource)sender;
				linqSource.GetQueryable -= new EventHandler<DevExpress.Data.Linq.GetQueryableEventArgs>(Data_GetQueryable);
				linqSource.DismissQueryable -= new EventHandler<DevExpress.Data.Linq.GetQueryableEventArgs>(Data_DismissQueryable);
			}
			GetQueryableEventArgs args = new GetQueryableEventArgs();
		...
		void ResetLinqInstantFeedbackSource() {
			if(Data != null) {
				LinqInstantFeedbackSource oldSource = (LinqInstantFeedbackSource)Data;
				oldSource.Dispose();
			}
			if(DesignerProperties.GetIsInDesignMode(this)) {
				Data = null;
			} else {
				LinqInstantFeedbackSource newSource = new LinqInstantFeedbackSource();
		...
		protected override string GetDesignTimeImageName() {
			return "DevExpress.Xpf.Core.Core.Images.LinqInstantFeedbackDataSource.png";
		}
		public LinqInstantFeedbackDataSource() {
			ResetLinqInstantFeedbackSource();
		...
		public void Refresh() {
			if(Data != null) {
				((LinqInstantFeedbackSource)Data).Refresh();
			}
		}
		public void Dispose() {
			isDisposed = true;
			if(Data != null) {
				((LinqInstantFeedbackSource)Data).Dispose();
			}
		}
	}
	public delegate void GetQueryableEventHandler(object sender, GetQueryableEventArgs e);
