...
namespace DevExpress.Xpf.Core.Design.Wizards.Mvvm.Resources.Views.WinForms.Outlook
{
	...
	public partial class ElementView_OutlookDesigner : ElementView_OutlookDesignerBase
	{
...
foreach(var mainCommand in viewModelData.Commands){
			...
			this.Write(this.ToStringHelper.ToStringWithCulture(viewModelData.EntityTypeName));
			this.Write(" Tasks\"\r\n\t\t\t\' \r\n\t\t\t\' ribbonPageGroupLayout\r\n\t\t\t\'\r\n\t\t\tMe.ribbonPageGroupLayout.All" +
					"owTextClipping = False\r\n\t\t\tMe.ribbonPageGroupLayout.ItemLinks.Add(Me.bbiCustomiz" +
					"e)\r\n\t\t\t");
foreach(var mainCommand in viewModelData.Commands){
				string nameForBarItemMain = mainCommand.CommandPropertyName.Remove(mainCommand.CommandPropertyName.Length -7,7);
				if(nameForBarItemMain.Contains("Layout")){
...
foreach(var commandLookUpTable in lookUpTable.Commands){
			...
			this.Write(" \r\n\t\t\t\'\r\n\t\t\t\'call RetrieveFields\r\n\t\t\t\'\r\n\t\t\tMe.dataLayoutControl1.RetrieveFields(p" +
					"arameters)\r\n\t\t\t\' \r\n            \' mvvmContext\r\n            \' \r\n            Me.mvv" +
					"mContext.ContainerControl = Me\r\n            Me.mvvmContext.ViewModelType = GetTy" +
					"pe(Global.");
			this.Write(this.ToStringHelper.ToStringWithCulture(mvvmContextFullName));
			this.Write(")\r\n\t\t\t");
foreach(var item in viewModelData.Commands){
...
foreach(var commandLookUpTable in lookUpTable.Commands){
	...
	#region Base class
	[System.CodeDom.Compiler.GeneratedCodeAttribute("Microsoft.VisualStudio.TextTemplating", "10.0.0.0")]
	public class ElementView_OutlookDesignerBase
	{
		#region Fields
