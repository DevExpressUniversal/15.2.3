...
namespace DevExpress.Xpf.Core.Design.Wizards.Mvvm.Resources.Views.WinForms.WinUI
{
	...
	public partial class ElementView_WinUI : ElementView_WinUIBase
	{
...
foreach(var lookUpTable in viewModelData.LookUpTables){
			...
			foreach(var mainCommand in viewModelData.Commands){
				string nameForItem = mainCommand.CommandPropertyName.Remove(mainCommand.CommandPropertyName.Length -7,7);
...
indexer++;}}
			this.Write("\t\t\tfluentAPI.BindCommand(((DevExpress.Utils.MVVM.ISupportCommandBinding)windowsUI" +
					"ButtonPanelCloseButton.Buttons[0]), x => x.Close());\t\r\n       }\r\n    }\r\n}\r\n");
}
if(IsVisualBasic){
...
foreach(var lookUpTable in viewModelData.LookUpTables){
			...
			foreach(var mainCommand in viewModelData.Commands){
				string nameForItem = mainCommand.CommandPropertyName.Remove(mainCommand.CommandPropertyName.Length -7,7);
				if(!nameForItem.Contains("Layout") && nameForItem != "Close"){
			this.Write("\t\t\tfluentAPI.BindCommand((DirectCast(windowsUIButtonPanelMain.Buttons(");
			this.Write(this.ToStringHelper.ToStringWithCulture(indexer));
			this.Write("), DevExpress.Utils.MVVM.ISupportCommandBinding)), Sub(x) x.[");
			this.Write(this.ToStringHelper.ToStringWithCulture(nameForItem));
			this.Write("]())\r\n\t\t\t");
indexer++;}}
			this.Write("\t\t\tfluentAPI.BindCommand((DirectCast(windowsUIButtonPanelCloseButton.Buttons(0), " +
					"DevExpress.Utils.MVVM.ISupportCommandBinding)), Sub(x) x.[Close]())\r\n\t\tEnd Sub\r\n" +
					"\tEnd Class\r\nEnd Namespace\r\n");
}
...
static string GetLookUpPropertyName(EntityViewModelData parentViewModelData, LookUpCollectionViewModelData viewModelData){
	...
	#region Base class
	[System.CodeDom.Compiler.GeneratedCodeAttribute("Microsoft.VisualStudio.TextTemplating", "10.0.0.0")]
	public class ElementView_WinUIBase
	{
		#region Fields
