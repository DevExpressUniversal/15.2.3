...
namespace DevExpress.Xpf.Core.Design.Wizards.Mvvm.Resources.Views.WinForms.WinUI
{
	...
	public partial class CollectionView_WinUI : CollectionView_WinUIBase
	{
...
namespace ");
			...
			this.Write(",\r\n                    (gView, entity) => gView.FocusedRowHandle = gView.FindRow(" +
					"entity));\r\n\t\t\t//We want to show ribbon print preview when bbiPrintPreview clicke" +
					"d\r\n\t\t\t");
			int indexer = 0;
			...
			{
			this.Write("\t\t\tfluentAPI.BindCommand(((DevExpress.Utils.MVVM.ISupportCommandBinding)windowsUI" +
					"ButtonPanel.Buttons[");
			...
			this.Write("\t\t\t");
indexer++;}
			this.Write("\t\t\t((DevExpress.XtraBars.Docking2010.WindowsUIButton)windowsUIButtonPanel.Buttons" +
					"[");
			this.Write(this.ToStringHelper.ToStringWithCulture(viewModelData.Commands.Count() + 1));
			this.Write("]).Click += (s, e) => { gridControl.ShowRibbonPrintPreview(); };\r\n        }\r\n    " +
					"}\r\n}\r\n");
}
if(IsVisualBasic){
...
Namespace Global.");
			...
			if(String.IsNullOrEmpty(item.ParameterPropertyName))
			{
			this.Write("\t\t\tfluentAPI.BindCommand((DirectCast(windowsUIButtonPanel.Buttons(");
			this.Write(this.ToStringHelper.ToStringWithCulture(indexer));
			this.Write("), DevExpress.Utils.MVVM.ISupportCommandBinding)), Sub(x) x.[");
			this.Write(this.ToStringHelper.ToStringWithCulture(nameForItem));
			this.Write("]())\r\n\t\t\t");
}
			this.Write("\t\t\t");
			...
			this.Write("\t\t\tfluentAPI.BindCommand((DirectCast(windowsUIButtonPanel.Buttons(");
			this.Write(this.ToStringHelper.ToStringWithCulture(indexer));
			this.Write("), DevExpress.Utils.MVVM.ISupportCommandBinding)), Sub(x, p) x.[");
			this.Write(this.ToStringHelper.ToStringWithCulture(nameForItem));
			this.Write("](p), Function(x) x.");
			this.Write(this.ToStringHelper.ToStringWithCulture(item.ParameterPropertyName));
			this.Write(")\r\n\t\t\t");
}
			this.Write("\t\t\t");
indexer++;}
			this.Write("\t\t\t\'We want to show ribbon print preview when windowsUIButton clicked\r\n\t\t\tAddHand" +
					"ler CType(windowsUIButtonPanel.Buttons(");
			this.Write(this.ToStringHelper.ToStringWithCulture(viewModelData.Commands.Count() + 1));
			this.Write("), DevExpress.XtraBars.Docking2010.WindowsUIButton).Click, Sub(s, e) gridControl." +
	...
	#region Base class
	[System.CodeDom.Compiler.GeneratedCodeAttribute("Microsoft.VisualStudio.TextTemplating", "10.0.0.0")]
	public class CollectionView_WinUIBase
	{
		#region Fields
