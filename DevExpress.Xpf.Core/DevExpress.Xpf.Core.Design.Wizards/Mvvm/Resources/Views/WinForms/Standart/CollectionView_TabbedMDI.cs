...
namespace DevExpress.Xpf.Core.Design.Wizards.Mvvm.Resources.Views.WinForms.Standart
{
	...
	public partial class CollectionView_TabbedMDI : CollectionView_TabbedMDIBase
	{
...
Namespace Global.");
			...
			this.Write(this.ToStringHelper.ToStringWithCulture(viewModelData.CollectionPropertyName));
			this.Write(" collection in grid and react on this collection external changes (Reload, server" +
					"-side Filtering)\r\n\t\t\tfluentAPI.SetBinding(gridControl, Function(gControl) gContr" +
					"ol.DataSource, Function(x) x.");
			this.Write(this.ToStringHelper.ToStringWithCulture(viewModelData.CollectionPropertyName));
			this.Write(")\r\n\t\t\t\' We want to show loading-indicator when data is loading asynchronously\r\n\t\t" +
					"\tfluentAPI.SetBinding(gridView, Function(gView) gView.LoadingPanelVisible, Funct" +
					"ion(x) x.IsLoading)\r\n\t\t\t");
			if(viewModelData.HasEntityEditProperty()){
			this.Write(@"			' We want to proceed the Edit command when row double-clicked
...
");
	...
	#region Base class
	[System.CodeDom.Compiler.GeneratedCodeAttribute("Microsoft.VisualStudio.TextTemplating", "10.0.0.0")]
	public class CollectionView_TabbedMDIBase
	{
		#region Fields
