...
namespace DevExpress.Charts.Designer.Native {
	public class ChangeFunnelRatioCommand : SeriesOptionsCommandBase, IComboBoxItememsFiller {
		...
		public override string Caption {
			get {
				return ChartDesignerLocalizer.GetString(ChartDesignerStringIDs.SeriesOptions_FunnelRatio);
			}
		}
		public override string ImageName {
			get { return null; }
		}
		public ChangeFunnelRatioCommand(WpfChartModel chartModel)
			: base(chartModel) { }
		protected override bool CanExecute(object parameter) {
			return SelectedSeries is FunnelSeries2D && !((FunnelSeries2D)SelectedSeries).HeightToWidthRatioAuto;
		}
		protected override object RuntimeRedo(HistoryItem historyItem) {
			...
			funnelSeries.HeightToWidthRatio = (double)historyItem.NewValue;
			return null;
		}
		protected override object RuntimeUndo(HistoryItem historyItem) {
			int seriesIndex = historyItem.ExecuteCommandInfo.IndexByNameDictionary[seriesIndexKey];
			var funnelSeries = (FunnelSeries2D)PreviewChart.Diagram.Series[seriesIndex];
			funnelSeries.HeightToWidthRatio = (double)historyItem.OldValue;
			return null;
		}
		...
		public override void DesigntimeApply(IModelItem chartModelItem, HistoryItem historyItem, IDesignTimeProvider designTimeProvider) {
			...
			seriesModelItem.Properties["HeightToWidthRatio"].SetValue(historyItem.NewValue);
		}
		...
		public override CommandResult RuntimeExecute(object parameter) {
			...
			double oldValue = SelectedSeriesModel.HeightToWidthRatio;
			SelectedSeriesModel.HeightToWidthRatio = newValue;
		...
		object[] IComboBoxItememsFiller.GetItems() {
			return new object[] { 0.25, 0.5, 1, 1.5, 1.75 };
		}
