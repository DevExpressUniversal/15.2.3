...
namespace DevExpress.Charts.Designer.Native {
	public class SetNestedDonutInnerIndentCommand : SeriesOptionsCommandBase {
		...
		public SetNestedDonutInnerIndentCommand(WpfChartModel chartModel)
			: base(chartModel) { }
		bool IsLastInGroup(NestedDonutSeries2D nestedDonutSeries) {
			if (PreviewChart.Diagram == null)
				return false;
			Dictionary<object, List<Series>> groupsDict = new Dictionary<object, List<Series>>();
			foreach (Series series in PreviewChart.Diagram.Series) {
				var nestedDonut = series as NestedDonutSeries2D;
				if (nestedDonut == null)
					continue;
				object group = nestedDonut.Group ?? DefaultGroup;
				List<Series> seriesGroup;
				bool isContained = groupsDict.TryGetValue(group, out seriesGroup);
				if (isContained)
					seriesGroup.Add(series);
				else {
					seriesGroup = new List<Series>() { series };
					groupsDict.Add(group, seriesGroup);
				}
			}
			List<Series> seriesGroupOfGivenSeries = groupsDict[nestedDonutSeries.Group ?? DefaultGroup];
			if (seriesGroupOfGivenSeries[seriesGroupOfGivenSeries.Count - 1] == nestedDonutSeries)
				return true;
			else
				return false;
		}
		protected override bool CanExecute(object parameter) {
			return (SelectedSeries is NestedDonutSeries2D) && !IsLastInGroup((NestedDonutSeries2D)SelectedSeries);
		}
		public override CommandResult RuntimeExecute(object parameter) {
			double newValue = Convert.ToDouble(parameter);
			double oldValue = SelectedSeriesModel.InnerIndent;
			SelectedSeriesModel.InnerIndent = newValue;
