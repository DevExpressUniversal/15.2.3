...
namespace DevExpress.Charts.Designer.Native {
	public class RemoveSeriesPointsCommand : SeriesOptionsCommandBase {
		...
		protected override void UndoInternal(WpfChartSeriesModel model, object oldValue) {
			model.Series.Points.AddRange((SeriesPointCollection)oldValue);
		}
		public override CommandResult RuntimeExecute(object parameter) {
			if (SeriesModel.Series.Points.Count > 0) {
				SeriesPointCollection oldValue = new SeriesPointCollection();
				oldValue.AddRange(SeriesModel.Series.Points);
				...
				SeriesModel.Series.Points.Clear();
