...
namespace DevExpress.Charts.Designer.Native {
	public class ChangeSeriesAxisYCommand : SeriesOptionsCommandBase {
		readonly string caption = ChartDesignerLocalizer.GetString(ChartDesignerStringIDs.SeriesOptions_ValueAxis);
		...
		public ChangeSeriesAxisYCommand(WpfChartModel model)
			: base(model) {
		}
		...
		protected override void RedoInternal(WpfChartSeriesModel model, object newValue) {
			...
			if (axisIndex == -1)
				model.AxisY = ChartModel.DiagramModel.PrimaryAxisModelY;
			else
				model.AxisY = (WpfChartAxisModel)ChartModel.DiagramModel.SecondaryAxesCollectionModelY.ModelCollection[axisIndex];
		}
		...
		protected override void UndoInternal(WpfChartSeriesModel model, object oldValue) {
			...
			if (oldAxisIndex == -1)
				...
				model.AxisY = (WpfChartAxisModel)ChartModel.DiagramModel.SecondaryAxesCollectionModelY.ModelCollection[oldAxisIndex];
		}
		[ SecuritySafeCritical ]
		public override void DesigntimeApply(IModelItem chartModelItem, HistoryItem historyItem, IDesignTimeProvider designTimeProvider) {
			...
			DXPropertyIdentifier axisYPropIdentifier = new DXPropertyIdentifier(typeof(XYDiagram2D), "SeriesAxisY");
			seriesAccess.Properties[axisYPropIdentifier].SetValue(bindingItem);
		}
		public override void RuntimeApply(ChartControl chartControl, HistoryItem historyItem) {
			...
			int axisIndex;
			SecondaryAxisY2D seriesAxis = null;
			if (historyItem.ExecuteCommandInfo.IndexByNameDictionary.TryGetValue(AxisIndexKey, out axisIndex))
				seriesAxis = ((XYDiagram2D)chartControl.Diagram).SecondaryAxesY[axisIndex];
			XYDiagram2D.SetSeriesAxisY((XYSeries)series, seriesAxis);
		}
		public override CommandResult RuntimeExecute(object parameter) {
			WpfChartAxisModel axisModel = (WpfChartAxisModel)parameter;
			int oldAxisIndex = SeriesModel.AxisY.GetSelfIndex();
			SeriesModel.AxisY = axisModel;
