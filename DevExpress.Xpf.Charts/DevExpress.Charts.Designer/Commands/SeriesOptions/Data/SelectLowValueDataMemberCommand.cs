...
namespace DevExpress.Charts.Designer.Native {
	public class SelectLowValueDataMemberCommand : SelectDataMemberCommandBase {
		public override string Caption {
			get { return ChartDesignerLocalizer.GetString(ChartDesignerStringIDs.SeriesOptions_LowValueDataMember); }
		}
		public SelectLowValueDataMemberCommand(WpfChartModel model)
			: base(model) {
		}
		...
		public override CommandResult DataMemberRuntimeExecute(object parameter) {
			...
			DataMemberInfo lowValueDataMember = parameter as DataMemberInfo;
			if (String.IsNullOrEmpty(lowValueDataMember.DataMember) || lowValueDataMember.DataMember == "None")
				lowValueDataMember = new DataMemberInfo();
			DataMemberInfo oldValue = SeriesModel.LowValueDataMember;
			if (lowValueDataMember != oldValue) {
				...
				SeriesModel.LowValueDataMember = lowValueDataMember;
				resultItem.HistoryItems.Add(new HistoryItem(new ExecuteCommandInfo(parameter, indexItems), this, null, oldValue, lowValueDataMember));
				if (SeriesModel.IsSeriesTemplate) {
					if (String.IsNullOrEmpty(lowValueDataMember.DataMember) && !AllDataMembersSet()) {
