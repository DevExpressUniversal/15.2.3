...
namespace DevExpress.Charts.Designer.Native {
	public class SelectCloseValueDataMemberCommand : SelectDataMemberCommandBase {
		public override string Caption {
			get { return ChartDesignerLocalizer.GetString(ChartDesignerStringIDs.SeriesOptions_CloseValueDataMember); }
		}
		public SelectCloseValueDataMemberCommand(WpfChartModel model)
			: base(model) {
		}
		...
		public override CommandResult DataMemberRuntimeExecute(object parameter) {
			...
			DataMemberInfo closeValueDataMember = parameter as DataMemberInfo;
			if (String.IsNullOrEmpty(closeValueDataMember.DataMember) || closeValueDataMember.DataMember == "None")
				closeValueDataMember = new DataMemberInfo();
			DataMemberInfo oldValue = SeriesModel.CloseValueDataMember;
			if (closeValueDataMember != oldValue) {
				...
				SeriesModel.CloseValueDataMember = closeValueDataMember;
				resultItem.HistoryItems.Add(new HistoryItem(new ExecuteCommandInfo(parameter, indexItems), this, null, oldValue, closeValueDataMember));
				if (SeriesModel.IsSeriesTemplate) {
					if (String.IsNullOrEmpty(closeValueDataMember.DataMember) && !AllDataMembersSet()) {
