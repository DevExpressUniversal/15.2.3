...
namespace DevExpress.Xpf.Charts.ModelSupport {
	public class RadarChartConfigurator : ChartConfigurator {
		new Model.RadarChart ChartModel { get { return base.ChartModel as Model.RadarChart; } }
		public RadarChartConfigurator(Model.Chart chartModel, Model.IModelElementContainer container)
			: base(chartModel, container) {
		}
		void ConfigureAxes(RadarDiagram2D diagram) {
			RadarAxesConfigurator configurator = new RadarAxesConfigurator(ChartModel, Container);
			configurator.Configure(diagram);
		}
		protected override void ConfigureDiagram(ChartControl chart) {
			RadarDiagram2D diagram = new RadarDiagram2D();
