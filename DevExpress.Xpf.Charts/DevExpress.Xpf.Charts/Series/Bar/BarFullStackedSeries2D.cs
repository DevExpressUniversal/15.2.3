...
namespace DevExpress.Xpf.Charts {
	public class BarFullStackedSeries2D : BarStackedSeries2D {
		public static readonly DependencyProperty PercentOptionsProperty = DependencyPropertyManager.RegisterAttached("PercentOptions", 
			typeof(PercentOptions), typeof(BarFullStackedSeries2D), new PropertyMetadata(PointOptions.PercentOptionsPropertyChanged));
		...
		public BarFullStackedSeries2D() {
			DefaultStyleKey = typeof(BarFullStackedSeries2D);
		}
		protected override Series CreateObjectForClone() {
			return new BarFullStackedSeries2D();
		}
		protected override SeriesContainer CreateContainer() {
			return new FullStackedInteractionContainer(this);
		}
		protected override IEnumerable<double> GetCrosshairValues(RefinedPoint refinedPoint) {
			yield return ((IFullStackedPoint)refinedPoint).MaxValue;
		}
