...
namespace DevExpress.Xpf.WindowsUI.Base {
	internal class DependencyPropertyRegistrator<OwnerType> where OwnerType : class {
		...
		public void RegisterEvent<THandler>(string name, ref RoutedEvent routedEvent, RoutingStrategy routingStrategy) {
			routedEvent = EventManager.RegisterRoutedEvent(name, routingStrategy, typeof(THandler), typeof(OwnerType));
		}
