...
namespace DevExpress.Xpf.WindowsUI.Base {
	public class veHeaderedContentControl : veContentControl {
		#region static
		...
		static veHeaderedContentControl() {
			var dProp = new DependencyPropertyRegistrator<veHeaderedContentControl>();
			dProp.Register("Header", ref HeaderProperty, (object)null,
				(dObj, e) => ((veHeaderedContentControl)dObj).OnHeaderChanged(e.OldValue, e.NewValue));
			dProp.Register("HeaderStringFormat", ref HeaderStringFormatProperty, (string)null,
				(dObj, e) => ((veHeaderedContentControl)dObj).OnHeaderStringFormatChanged((string)e.NewValue));
			dProp.Register("HeaderTemplate", ref HeaderTemplateProperty, (DataTemplate)null,
				(dObj, e) => ((veHeaderedContentControl)dObj).OnHeaderTemplateChanged((DataTemplate)e.NewValue));
			dProp.Register("HeaderTemplateSelector", ref HeaderTemplateSelectorProperty, (DataTemplateSelector)null,
				(dObj, e) => ((veHeaderedContentControl)dObj).OnHeaderTemplateSelectorChanged());
			...
			dProp.Register("HasHeader", ref HasHeaderProperty, (bool)false,
				(dObj, e) => ((veHeaderedContentControl)dObj).OnHasHeaderChanged());
			dProp.Register("HasHeaderTemplate", ref HasHeaderTemplateProperty, (bool)false,
				(dObj, e) => ((veHeaderedContentControl)dObj).OnHasHeaderTemplateChanged());
		}
		#endregion
		protected internal veHeaderedContentControl() {
		...
		void PrepareHeaderedContentControl(object item, DataTemplate itemTemplate, DataTemplateSelector itemTemplateSelector) {
			if(item != this) {
				if(!(item is UIElement) && Header == null) {
					Header = item;
				}
				if(itemTemplate != null && HeaderTemplate == null) {
					base.SetValue(HeaderTemplateProperty, itemTemplate);
				}
				if(itemTemplateSelector != null && HeaderTemplateSelector == null) {
	...
	class HeaderedControlHelper {
		public static object GetActualHeader(object header, string headerFormat, string language) {
			if(!string.IsNullOrEmpty(headerFormat) && header != null) {
				var culture = new System.Globalization.CultureInfo(language);
				if(header is string || header is System.DateTime)
					return string.Format(culture, headerFormat, header);
				if(header.GetType().IsPrimitive)
					return string.Format(culture, headerFormat, header);
			}
			return header;
		}
