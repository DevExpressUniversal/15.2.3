...
namespace DevExpress.Xpf.Grid {
	public class DateSmartColumnFilterInfo : DateColumnFilterInfo {
		readonly DateFilterCache cache;
		public DateSmartColumnFilterInfo(ColumnBase column) : base(column) {
			cache = new DateFilterCache(Grid, column);
		}
		...
		protected override void UpdatePopupData(PopupBaseEdit popup, object[] values) { 
			cache.UpdateValuesForColumn(values);
			UpdateBottomFilters();
			CalendarFilter.RebuildFilters();
		}
		protected override FilterData[] CreateBottomFilters() {
			var filters = base.CreateBottomFilters();
			return filters.Where(x => cache.IsFilterVisibleForColumn(x)).ToArray();
		}
	}
}
