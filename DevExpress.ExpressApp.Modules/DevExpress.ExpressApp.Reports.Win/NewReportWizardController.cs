...
namespace DevExpress.ExpressApp.Reports.Win {
	public class NewReportWizardController : ViewController {
		...
		private void newObjectController_ObjectCreating(object sender, ObjectCreatingEventArgs e) {
			if(!typeof(IReportData).IsAssignableFrom(e.ObjectType)) {
				return;
			}
			e.Cancel = true;
		...
		protected virtual void ShowNewReportWizard(Type reportDataType) {
			Frame.GetController<WinReportServiceController>().ShowWizard(reportDataType);
		}
		protected override void OnActivated() {
			base.OnActivated();
			if(ReportsModule.ActivateReportController(this)) {
