module __aspxRichEdit {
    export class ShowAllFieldResultCommand extends CommandBase<IntervalCommandState> {
        ...
        executeCore(state: IntervalCommandState, parameter: any): boolean {
            ...
            for (var subDocumentIndex: number = 0, currSubDoc: SubDocument; currSubDoc = subDocumentsList[subDocumentIndex]; subDocumentIndex++) {
                ...
                if (fieldIndexStartRecalculateLayout != null) {
                    ...
                    this.control.formatterOnIntervalChanged(
                        FixedInterval.fromPositions(fields[fieldIndexStartRecalculateLayout].getFieldStartPosition(), fields[fields.length - 1].getFieldEndPosition()), currSubDoc);
                }
            }
