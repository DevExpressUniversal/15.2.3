...
namespace DevExpress.Web.ASPxRichEdit.Internal {
	public class InsertInlinePictureCommand : WebRichEditUpdateModelCommandBase {
		public InsertInlinePictureCommand(CommandManager commandManager, Hashtable parameters) : base(commandManager, parameters) { }
		...
		protected override void PerformModifyModel() {
			...
			int scaleX = Convert.ToInt32(Parameters["scaleX"]);
			int scaleY = Convert.ToInt32(Parameters["scaleY"]);
			int id = (int)Parameters["id"];
			OfficeImage image = DocumentModel.ImageCache.GetImageByKey(id);
			PieceTable.InsertInlinePicture(logPosition, image, scaleX, scaleY);
		}
