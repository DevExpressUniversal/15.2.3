...
namespace DevExpress.Web.ASPxRichEdit.Internal {
	public class LoadParagraphStylesCommand : WebRichEditLoadModelCommandBase {
		public LoadParagraphStylesCommand(WebRichEditLoadModelCommandBase parentCommand, Hashtable parameters)
		...
		protected override void FillHashtable(Hashtable result) {
			WebParagraphStyleCollection paragraphStyles = new WebParagraphStyleCollection();
			...
			foreach (var modelStyle in DocumentModel.ParagraphStyles) {
				WebParagraphStyle style = new WebParagraphStyle(modelStyle);
			...
			result.Add("paragraphStyles", paragraphStyles.ToHashtableCollection());
