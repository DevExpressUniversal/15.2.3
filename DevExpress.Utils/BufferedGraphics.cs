	...
	public sealed class XtraBufferedGraphics : IDisposable {
		...
		public void Render() {
			if(targetGraphics != null)
				...
				RenderInternal(new HandleRef(Graphics, targetDC), this, 0);
		}
		public void RenderFrame(Rectangle client) {
			if(targetGraphics != null)
				RenderFrame(targetGraphics, client);
			else
				RenderFrameInternal(new HandleRef(Graphics, targetDC), this, 0, client);
		}
		public void Render(Graphics target, Point targetPoint, int rop) {
			this.targetLoc = targetPoint;
			if(target != null) {
				IntPtr graphicsHdc = target.GetHdc();
				try {
					this.RenderInternal(new HandleRef(target, graphicsHdc), this, rop);
				}
				finally { target.ReleaseHdcInternal(graphicsHdc); }
			}
		}
		public void Render(Graphics target) {
			if(target != null) {
				IntPtr graphicsHdc = target.GetHdc();
				try {
					this.RenderInternal(new HandleRef(target, graphicsHdc), this, 0);
				}
				finally { target.ReleaseHdcInternal(graphicsHdc); }
			}
		}
		public void RenderFrame(Graphics target, Rectangle client) {
			if(target != null) {
				IntPtr graphicsHdc = target.GetHdc();
				try {
					this.RenderFrameInternal(new HandleRef(target, graphicsHdc), this, 0, client);
				}
				finally { target.ReleaseHdcInternal(graphicsHdc); }
			}
		}
		...
		void RenderInternal(HandleRef refTargetDC, XtraBufferedGraphics buffer, int rop) {
			if(rop == 0) rop = XtraBufferedGraphics.rop;
			IntPtr graphicsHdc = buffer.Graphics.GetHdc();
			try {
				NativeMethods.BitBlt(refTargetDC, this.targetLoc.X, this.targetLoc.Y, this.virtualSize.Width, this.virtualSize.Height, new HandleRef(buffer.Graphics, graphicsHdc), 0, 0, rop);
			}
			finally { buffer.Graphics.ReleaseHdcInternal(graphicsHdc); }
		}
		void RenderFrameInternal(HandleRef refTargetDC, XtraBufferedGraphics buffer, int rop, Rectangle client) {
			if(rop == 0) rop = XtraBufferedGraphics.rop;
			IntPtr graphicsHdc = buffer.Graphics.GetHdc();
			try {
				Rectangle bounds = new Rectangle(targetLoc, virtualSize);
				NativeMethods.BitBlt(refTargetDC, bounds.Left, bounds.Top, bounds.Width, client.Top - bounds.Top, new HandleRef(buffer.Graphics, graphicsHdc), 0, 0, rop);
				NativeMethods.BitBlt(refTargetDC, bounds.Left, client.Top, client.Left - bounds.Left, client.Height, new HandleRef(buffer.Graphics, graphicsHdc), 0, client.Top, rop);
				NativeMethods.BitBlt(refTargetDC, client.Right, client.Top, bounds.Right - client.Right, client.Height, new HandleRef(buffer.Graphics, graphicsHdc), client.Right, client.Top, rop);
				NativeMethods.BitBlt(refTargetDC, bounds.Left, client.Bottom, bounds.Width, bounds.Bottom - client.Bottom, new HandleRef(buffer.Graphics, graphicsHdc), 0, client.Bottom, rop);
			}
			finally { buffer.Graphics.ReleaseHdcInternal(graphicsHdc); }
		}
