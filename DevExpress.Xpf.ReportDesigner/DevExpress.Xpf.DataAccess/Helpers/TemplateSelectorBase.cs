...
namespace DevExpress.Xpf.DataAccess.Native {
	public class TemplateSelectorBase : DataTemplateSelector {
		...
		protected DataTemplate GetDataTemplate(Type type) {
			if(type.IsEnum) return ListBoxTemplate;
			if(type == typeof(bool)) return CheckBoxTemplate;
			if(type == typeof(DateTime)) return DateTimeTemplate;
			return TextEditTemplate;
		}
	}
}
