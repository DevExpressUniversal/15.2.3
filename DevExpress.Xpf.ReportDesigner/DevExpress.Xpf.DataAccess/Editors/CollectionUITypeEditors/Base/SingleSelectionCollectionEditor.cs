...
namespace DevExpress.Xpf.DataAccess.Editors.CollectionUITypeEditors {
	public abstract class SingleSelectionCollectionEditor : CollectionUITypeEditorBase {
		...
		protected override void MoveItems(bool moveDown) {
			DoWithSelectedItem(x => Items.Swap(x, moveDown ? x + 1 : x - 1), true);
		}
		protected override bool CanExecuteMoveUpCommand() {
			return GetItemsList().IndexOf(SelectedItem) > 0;
		}
		protected override  bool CanExecuteMoveDownCommand() {
			var items = GetItemsList();
			return GetItemsList().IndexOf(SelectedItem) < items.Count - 1;
		}
