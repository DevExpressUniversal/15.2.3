...
namespace DevExpress.Data.XtraReports.Wizard.Presenters {
	public class SelectColumnsPage<TModel> : ReportWizardServiceClientPage<ISelectColumnsPageView, TModel> where TModel : ReportModel {
		...
		public override void Begin() {
			...
			RefreshView();
			Client.GetColumnsCompleted += Client_GetColumnsCompleted;
			Client.GetColumnsAsync(Model.DataSourceName, Model.DataMemberName, PageSessionId);
		}
		void Client_GetColumnsCompleted(object sender, ScalarOperationCompletedEventArgs<IEnumerable<ColumnInfo>> e) {
			Client.GetColumnsCompleted -= Client_GetColumnsCompleted;
			if(!object.Equals(e.UserState, PageSessionId))
				return;
			View.ShowWaitIndicator(false);
			if(HandleError(e, "Failed to retrieve a list of available columns."))
				return;
			availableColumns.AddRange(e.Result);
			selectedColumns.Clear();
