	...
	public class TypeDescriptorHelper {
		public static PropertyDescriptor CreateProperty(Type type, PropertyDescriptor oldPropertyDescriptor, Attribute[] attributes) {
			if(oldPropertyDescriptor == null)
				return null;
			AttributeCollection oldAttrs = oldPropertyDescriptor.Attributes;
			int count = oldAttrs.Count;
			Attribute[] attrs = new Attribute[count];
			for (int i = 0; i < count; i++)
				attrs[i] = oldAttrs[i];
			return TypeDescriptor.CreateProperty(type, oldPropertyDescriptor, attrs);
		}
	}
}
