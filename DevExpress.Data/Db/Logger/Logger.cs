...
namespace DevExpress.Xpo.Logger {
	...
	public class LoggerBase: ILogger, ILogSource {
		...
		Queue<LogMessage> log = new Queue<LogMessage>();
		public int Count {
		...
		public void Log(LogMessage message) {
			if (!enabled) return;
			lock (sync) {
				while (log.Count >= capacity) {
					log.Dequeue();
					lostMessageCount++;
					if (lostMessageCount == int.MaxValue) lostMessageCount = 0;
				}
				log.Enqueue(message);				
			}
		}
		...
		public LogMessage GetMessage() {
			...
			lock(sync) {
				if(log.Count > 0) return log.Dequeue();
				return null;
			}
		}
		public LogMessage[] GetMessages(int messagesAmount) {
			...
			lock(sync) {
				...
				int upperBound = log.Count;
				if(messagesAmount < log.Count) upperBound = messagesAmount;
				for(int i = 0; i < upperBound; i++) {
					result.Add(log.Dequeue());
				}
