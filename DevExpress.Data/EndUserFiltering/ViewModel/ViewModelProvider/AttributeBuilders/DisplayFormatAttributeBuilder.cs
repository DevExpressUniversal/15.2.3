...
namespace DevExpress.Utils.Filtering.Internal {
	...
	static class DisplayFormatAttributeBuilder {
		...
		static DisplayFormatAttributeBuilder() {
			try {
				DataAnnotationAttributeBuilderHelper.CheckDataAnnotations_ConditionallyAPTCAIssue<DisplayFormatAttribute>();
				var attributeType = typeof(DisplayFormatAttribute);
				attributeCtor = attributeType.GetConstructor(Type.EmptyTypes);
				attributeProperties = new PropertyInfo[] {
					attributeType.GetProperty("DataFormatString"),
				};
			}
		...
		internal static CustomAttributeBuilder Build(IEndUserFilteringMetric metric) {
			if(string.IsNullOrEmpty(metric.DataFormatString)) return null;
			return DataAnnotationAttributeBuilderHelper.Build(
				attributeCtor, attributeProperties,
				new object[] { 
					metric.DataFormatString 
				});
		}
	}
}
