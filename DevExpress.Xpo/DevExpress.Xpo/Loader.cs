...
namespace DevExpress.Xpo.Helpers {
	...
	internal class AsyncExecuteQueue {
		...
		public void Invoke(SynchronizationContext syncContext, SendOrPostCallback d, object state, bool operationEnd) {
			...
			} while(!DevExpress.Data.Utils.Helpers.WaitOne(invokeInfo.OperationEndEvent, 100));			
		...
		class ExecuteInvokeInfo {
			SendOrPostCallback d;
			object state;
			bool executed = false;
			bool operationEnd;
			Exception error;
			ManualResetEvent operationEndEvent = new ManualResetEvent(false);
	...
	class ObjectCollectionLoader {
		...
		Dictionary<XPClassInfo, StubMap> stubObjects = new Dictionary<XPClassInfo, StubMap>();
		Dictionary<XPClassInfo, IDictionary<object, object>> delayedObjects = new Dictionary<XPClassInfo, IDictionary<object, object>>();
		...
		public static bool AcceptLoadPropertyAndResetModified(bool trackPropertiesModifications, OptimisticLockingReadMergeBehavior loadMerge, object currentObject, XPMemberInfo member, Func<object, object> getNewValue, object getNewValueArgument) {
			OptimisticLockingReadMergeBehavior currentMergeBehavior = member.MergeCollisionBehavior == OptimisticLockingReadMergeBehavior.Default ? loadMerge : member.MergeCollisionBehavior;
		...
		public object LoadObjectsAsync(ObjectsQuery[] queries, AsyncLoadObjectsCallback callback, SynchronizationContext syncContext) {
			if (callback == null || syncContext == null) throw new ArgumentNullException();
			...
			try {
				...
				if(loadObjectsResult.Count == 0 && asyncLoadObjectsContext.Queries.Length > 0) {
					for(int i = 0; i < asyncLoadObjectsContext.Queries.Length; i++) {
						loadObjectsResult.Add(new object[0]);
					}
				}
		...
		LoadObjectsContext PrepareLoadObjects(ObjectsQuery[] queries) {
			...
			for(int i = 0; i < queries.Length; i++) {
				...
				if(expanded[i].IsFalse || Session.UpdateSchema(true, loadClassInfo) == UpdateSchemaResult.FirstTableNotExists)
					...
					if(queries[i].Sorting != null) {
						foreach(SortProperty sp in queries[i].Sorting) {
					...
					int skipPhisicallySelected = queries[i].SkipSelectedRecords;
					int topPhisicallySelected = queries[i].TopSelectedRecords;
					if(topPhisicallySelected + skipPhisicallySelected != 0 && expanded[i].RequiresPostProcessing || requiresResorting[i]) {
						topPhisicallySelected = 0;
						skipPhisicallySelected = 0;
					}
					loadDataQuery.Add(new ObjectsQuery(loadClassInfo, expanded[i].ExpandedCriteria,
						srt, skipPhisicallySelected, topPhisicallySelected, queries[i].CollectionCriteriaPatcher, queries[i].Force));
				}
			}
		...
		object ProcessStub(StubMap map, LoadStub stub) {
			if (stub.IsProceed) return stub.LoadedObject;
			stub.Proceed();
			...
			foreach (XPMemberInfo mi in stub.Members) {
				...
				if (memberPASData != null) {
					...
					if (!AcceptLoadPropertyAndResetModified(Session.TrackPropertiesModifications, stub.LoadMerge, stub.LoadedObject, mi, ReturnArgument, newData))
						continue;
					...
					if (!AcceptLoadPropertyAndResetModified(Session.TrackPropertiesModifications, stub.LoadMerge, stub.LoadedObject, mi, ReturnArgument, memberValue))
						continue;
		...
		class ObjectLoader {
			...
			void InternalLoadObject(object id, OptimisticLockingReadMergeBehavior loadMerge) {
				...
				if (!useStubs) {
					...
					if (loadMerge != OptimisticLockingReadMergeBehavior.Default) ((LoadStub)theObject).LoadMerge = loadMerge;
					loader.objectsToFireLoaded.Add(theObject);
			...
			void LoadProperties(OptimisticLockingReadMergeBehavior loadMerge) {
				foreach(var member in members) {
					if (!AcceptLoadPropertyAndResetModified(loader.Session.TrackPropertiesModifications, loadMerge, theObject, member.Member, ReturnMemberValue, member))
						continue;
					member.AcquireValue();
				}
...
#if CF
	internal class SynchronizationContext {	   
		public static SynchronizationContext Current{
			get { return new SynchronizationContext(); }
		}
		System.Windows.Forms.Control syncControl = new System.Windows.Forms.Control();
		public void Send(SendOrPostCallback callback, object param) {
			syncControl.Invoke(callback, param);
		}
		public void Post(SendOrPostCallback callback, object param) {
			syncControl.BeginInvoke(callback, param);
		}
	}
	internal delegate void SendOrPostCallback(object param);
#endif
}
