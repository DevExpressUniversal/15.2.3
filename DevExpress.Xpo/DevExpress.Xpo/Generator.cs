...
namespace DevExpress.Xpo.Generators {
				...
				if(classInfo.TableMapType == MapInheritanceType.ParentTable) {
					if(classInfo.IsTypedObject) {
						...
						foreach(XPObjectType objectType in TypesPatchManager.AllTypes.Values) {
							if(objectType.TypeClassInfo.IsAssignableTo(classInfo)) {
	...
	public class BatchWideDataHolder4Modification: BatchWideDataHolder {
...
#if DEBUGTEST && !SL
			NUnit.Framework.Assert.IsNull(deletedObjects);
#endif
		...
		public ParameterValue CreateIdentityParameter(object theObject) {
			...
			NUnit.Framework.Assert.IsFalse(identityParameters.ContainsKey(theObject));
#endif
...
#if DEBUGTEST && !SL
			NUnit.Framework.Assert.IsFalse(insertedObjects.Contains(theObject));
#endif
	...
	public class BatchWideDataHolder4Select: BatchWideDataHolder {
		...
		Dictionary<object, ParameterValue> parametersByValues;
		...
		public override OperandValue GetParameter(object value) {
			...
			if(ci == null) {
				if(parametersByValues == null)
					parametersByValues = new Dictionary<object, ParameterValue>();
				ParameterValue result;
				if(!parametersByValues.TryGetValue(value, out result)) {
					result = new ParameterValue(GetNextTag());
					result.Value = value;
					parametersByValues.Add(value, result);
				}
				return result;
			} else {
	...
	struct SingleAggregateItem {
		...
		public SingleAggregateItem(CriteriaOperator condition, XPClassInfo classInfo) {
			Condition = condition;
			ClassInfo = classInfo;
		}
	}
	class SingleAggregateItemComparer : IEqualityComparer<SingleAggregateItem> {
		public bool Equals(SingleAggregateItem x, SingleAggregateItem y) {
			return x.ClassInfo == y.ClassInfo && CriteriaOperator.CriterionEquals(x.Condition, y.Condition);
		}
		public int GetHashCode(SingleAggregateItem obj) {
			return (obj.ClassInfo == null ? 0x1984652 : obj.ClassInfo.GetHashCode()) ^ (ReferenceEquals(obj.Condition, null) ? 0x54382732 : obj.Condition.GetHashCode());
		}
	}
	public abstract class BaseQueryGenerator : IClientCriteriaVisitor<CriteriaOperator> {
		...
		JoinNode AppendInheritanceJoinNode(XPClassInfo branch, XPMemberInfo property, JoinNode prevnode) {
			...
			for(int i = 0; i < classes.Count; i++) {
				...
				if(prevnode.Table.Equals(mappingTable)) {
					...
					if(nakedSingleNodes != null && nakedSingleNodes.ContainsKey(prevnode.Alias)) {
		...
		JoinNode AppendSingleAggregateJoinNode(XPClassInfo ci, XPMemberInfo collectionProperty, CriteriaOperator condition, JoinNode prevnode, JoinType type) {
			...
			try {
				...
				SingleAggregateItem item = new SingleAggregateItem(condition, ci);
				JoinNode singleNode;
		...
		static string[] nodeAliasCache = new string[0];
		...
		class ReferenceComparer : IEqualityComparer<CriteriaOperator> {
			...
			public int GetHashCode(CriteriaOperator obj) {
				if(ReferenceEquals(obj, null)) return 0x0343435;
		...
		public CriteriaOperator ProcessSingleAggregateOperand(OperandProperty collectionProperty, CriteriaOperator condition, CriteriaOperator aggregatedExpression) {
			if(ReferenceEquals(collectionProperty, null)) throw new ArgumentNullException("collectionProperty");
			return ExecuteWithPropertyNameDiving(collectionProperty.PropertyName, (pn) => {
				MemberInfoCollection mic = ClassInfo.ParsePersistentPath(pn);
...
#pragma warning restore 618
			...
			try {
				...
				if(ReferenceEquals(theOperator.LeftOperand, null) || ReferenceEquals(theOperator.RightOperand, null))
					throw new ArgumentNullException("theOperator", Xpo.Res.GetString(Xpo.Res.Generator_OneOfBinaryOperatorsOperandsIsNull));
				var left = Process(theOperator.LeftOperand);
				var right = Process(theOperator.RightOperand);
...
value is ArrayList ||
 ...
 value is List<object>) {
			...
			return GetConstParameter(value);
		}
		protected OperandValue GetConstParameter(object value) {
			if(value == null)
				return new OperandValue();
			OperandValue parameter = BatchWideData.GetParameter(value);
			return parameter;
		}
			...
			try {
				if(theOperator.OperatorType == FunctionOperatorType.Custom && theOperator.Operands.Count == 3) {
					...
					if(exactType || instanceOfType) {
						...
						MemberInfoCollection propertyPath = ClassInfo.ParsePersistentPath(property.PropertyName);
						if(propertyPath[propertyPath.Count - 1].IsKey) {
							propertyPath = new MemberInfoCollection(ClassInfo, propertyPath.ToArray());
							propertyPath.RemoveAt(propertyPath.Count - 1);
						}
						string propertyStart = propertyPath.ToString();
						XPClassInfo searchedClassInfo;
						XPClassInfo objectClassInfo = propertyPath.Count > 0 ? propertyPath[propertyPath.Count - 1].ReferenceType : ClassInfo;
						...
						if(exactType) {
							return Process(new BinaryOperator(propertyName, TypesManager.GetObjectType(searchedClassInfo)));
						} else {
							CriteriaOperator result = null;
								List<XPObjectType> types = new List<XPObjectType>();
								foreach(XPObjectType objectType in TypesManager.AllTypes.Values) {
									if(objectType.TypeClassInfo.IsAssignableTo(searchedClassInfo)) {
										types.Add(objectType);
									}
								}
								switch(types.Count) {
					...
					if(theOperator.OperatorType == FunctionOperatorType.IsNull || theOperator.OperatorType == FunctionOperatorType.IsNullOrEmpty || theOperator.OperatorType == FunctionOperatorType.Iif)
						currentJoinType = JoinType.LeftOuter;
				...
				return new FunctionOperator(theOperator.OperatorType, processedOperands);
			} finally {
				currentJoinType = prevJoinType;
			}
		}
	...
	public abstract class BaseObjectQueryGenerator : BaseQueryGenerator {
		...
		OperandValue GetMemberParameter(XPMemberInfo member, object theObject) {
			...
			if(member.ReferenceType != null)
				return GetMemberParameter(member.ReferenceType.KeyProperty, member.GetValue(theObject));
	...
	public class StatementNormalizer {
		...
		void FindNode(JoinNode subNode, JoinNode parentNode) {
			...
			if(missedNode != null) {
				...
				if(collectedNodes.ContainsKey(missedNode)) {
					...
					NormalizeNodeTree(parentNode, subNode, collectedNodes);
					return;
				}
			}
			FindNodes(subNode);
		...
		void NormalizeNodeTree(JoinNode parentNode, JoinNode subNode, Dictionary<string, JoinNode> collectedNodes) {
			...
			PathInfo[] result = PathFinder.Find(parentNode.Alias, collectedNodes.Keys, criteriaList, nodeRelations);
		...
		class PathInfo {
			...
			public CriteriaOperator GetCriteria() {
				...
				if(criteriaList.Count == 0) return criteriaList[0];
		...
		class PathFinder {
			...
			readonly Dictionary<string, bool> leftNodes = new Dictionary<string, bool>();
			readonly Dictionary<string, bool> usedNodes = new Dictionary<string, bool>();
			public PathFinder(string rootNode, IEnumerable<string> nodes, IEnumerable<PlanAliasCriteriaInfo> criteriaList, Dictionary<string, List<string>> nodeRelations) {
				this.rootNode = rootNode;
				usedNodes.Add(rootNode, true);
			...
			public static PathInfo[] Find(string rootNode, IEnumerable<string> nodes, IEnumerable<PlanAliasCriteriaInfo> criteriaList, Dictionary<string, List<string>> nodeRelations) {
				return new PathFinder(rootNode, nodes, criteriaList, nodeRelations).Find();
			}
			...
			public bool ProcessNode(string currentNode) {
				...
				if(nodeRelations.TryGetValue(currentNode, out relations)) {
					foreach(string rel in relations) {
						if(!leftNodes.ContainsKey(rel)) continue;
			...
			public void ProcessCriteria(PathInfo pi) {
				for(int i = leftCriteriaList.Count - 1; i >= 0; i--) {
					...
					for(int j = 0; j < aliases.Length; j++) {
						...
						if(!usedNodes.ContainsKey(alias)) {
