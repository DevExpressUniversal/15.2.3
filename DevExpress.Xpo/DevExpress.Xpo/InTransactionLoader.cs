...
namespace DevExpress.Xpo.Helpers {
	public class InTransactionLoader: IPersistentValueExtractor {
		enum InTransactionState {
			...
			GetOriginalObjects,
			AfterGetOriginalObjectsThroughtCollection,
			ReturnResult,
		...
		public object GetObjectsAsync(ObjectsQuery[] queries, AsyncLoadObjectsCallback callback) {
			this.mainRequest = new VirtualAsyncRequest(SynchronizationContext.Current);
		...
		public object SelectDataAsync(XPClassInfo classInfo, CriteriaOperatorCollection properties, CriteriaOperator criteria, CriteriaOperatorCollection groupProperties, CriteriaOperator groupCriteria, bool selectDeleted, int skipSelectedRecords, int topSelectedRecords, SortingCollection sorting, AsyncSelectDataCallback callback) {
			...
			objectsToDelete = session.GetObjectsToDelete(true);
			if(objectsToSave.Count == 0 && objectsToDelete.Count == 0) {
				return session.SelectDataAsyncInternal(classInfo, properties, criteria, groupProperties, groupCriteria, selectDeleted, skipSelectedRecords, topSelectedRecords, sorting, callback);
			}
			ObjectsQuery originalQuery = GetQueryForSelectData(classInfo, criteria, selectDeleted, skipSelectedRecords, topSelectedRecords, sorting);
			List<object[]> result = session.PrepareSelectData(classInfo, ref properties, ref criteria, ref groupProperties, ref groupCriteria, ref sorting, false, this);
			if(result == null) {
		...
		void Process() {
			switch(currentState) {
				...
				case InTransactionState.AfterAnalyzeQueryDirectGetObjects:
					...
					break;
				case InTransactionState.AfterGetOriginalObjectsThroughtCollection:
		...
		void ExecDirectSelectData() {
			...
			if((currentMode & InTransactionMode.Async) == InTransactionMode.Async) {
				mainRequest.AddNestedRequest(session.SelectDataAsyncInternal(selectDataState.PreparedQuery.ClassInfo,
					selectDataState.Properties,
					selectDataState.PreparedQuery.Criteria,
					selectDataState.GroupProperties,
					selectDataState.GroupCriteria,
		...
		void StateNextQuery() {
			...
			if(loadObjectsState.QueryIndex >= loadObjectsState.Queries.Length) {
				if((currentMode & InTransactionMode.SelectData) == InTransactionMode.SelectData) {
		...
		List<object[]> ProcessPostProcessingGrouped() {
			...
			if(groupEvaluators.Length == 0) {
				groupingResult.Add(new SelectDataGroup(new object[] { }), new List<object>());
			}
		...
		void ProcessAnalyzeAndExecQuery() {
			try {
				...
				ExecQueries(new ObjectsQuery[] { new ObjectsQuery(analyzeResult.NodeClassInfoDict[node], reverseCriteriaDict[node], null, 0, 0, objectsQuery.CollectionCriteriaPatcher, objectsQuery.Force) },
		...
		void SaveCurrentResult(ICollection result, CriteriaOperator actualCondition) {
			...
			loadObjectsState.Result[loadObjectsState.QueryIndex] = FilterObjects(currentQuery.ClassInfo, result, objectsToSave,
				actualCondition, currentQuery.Sorting, currentQuery.SkipSelectedRecords, currentQuery.TopSelectedRecords, currentQuery.CollectionCriteriaPatcher.SelectDeleted);
		}
		void ProcessGetOriginalObjects() {
			try {
				...
				if(getOriginalObjectsState == null) {
					...
					if(path == null) throw new InvalidOperationException(Res.GetString(Res.CriteriaAnalizer_PathNotFound, loadObjectsState.QueryNode, "N0"));
					getOriginalObjectsState = new GetOriginalObjectsState(path);
					ObjectsQuery query = queriesForExecute[0];
					currentCollection = FilterObjects(query.ClassInfo, queriesForExecuteResult[0], objectsToSave, query.Criteria, query.CollectionCriteriaPatcher.SelectDeleted);
				} else {
				...
				if(pathItem.TransitionInfo.IsMemberInfo) {
					if(pathItem.TransitionInfo.MemberInfo.IsAssociationList && pathItem.Type == AnalyzeNodePathItemType.Reverse) {
						...
						ExecQueries(subQueries.ToArray(), InTransactionState.AfterGetOriginalObjectsThroughtCollection);
						return;
					} else {
						throw new InvalidOperationException();
					}
		...
		void ProcessAfterGetOriginalObjectsThroughtCollection() {
			try {
				...
				for(int c = 0; c < nextResult.Length; c++) {
					ICollection addCollection = FilterObjects(queriesForExecute[c].ClassInfo, nextResult[c], objectsToSave, queriesForExecute[c].Criteria, queriesForExecute[c].CollectionCriteriaPatcher.SelectDeleted);
		...
		ICollection FilterObjects(XPClassInfo classInfo, ICollection objects, ICollection objectsToSave, CriteriaOperator filterCriteria, bool selectDeleted) {
			return FilterObjects(classInfo, objects, objectsToSave, filterCriteria, null, 0, 0, selectDeleted);
		}
		ICollection FilterObjects(XPClassInfo classInfo, ICollection objects, ICollection objectsToSave, CriteriaOperator filterCriteria, SortingCollection sorting, int skipRecords, int topRecords, bool selectDeleted) {
			...
			foreach(object theObject in objects) {
				if(session.IsObjectToDelete(theObject, true) && !selectDeleted)
					continue;
