	...
	public abstract class StronglyTypedCriteriaVisitorBase<T> : IClientCriteriaVisitor<T> {
		...
		protected StronglyTypedCriteriaVisitorBase(XPClassInfo[] upLevelsClassInfos) {
			if(upLevelsClassInfos == null)
				throw new ArgumentNullException("upLevelsClassInfos");
	...
	public abstract class UnknownCriteriaEleminatorBase: StronglyTypedCriteriaVisitorBase<ExpandedCriteriaHolder> {
		...
		protected override ExpandedCriteriaHolder Visit(AggregateOperand theOperand) {
			...
			if(processedConditionHolder.IsFalse)
				return GetFalseConditionAggregate(theOperand.AggregateType);
		...
		protected override ExpandedCriteriaHolder Visit(JoinOperand theOperand) {			
			...
			if(processedConditionHolder.IsFalse)
				return GetFalseConditionAggregate(theOperand.AggregateType);
			CriteriaOperator persistentOperator = new JoinOperand(nestedUpLevels[0].FullName,
				ExpandedCriteriaHolder.IfNeededConvertToLogicalOperator(processedConditionHolder.ExpandedCriteria), 
				theOperand.AggregateType,
				ReferenceEquals(null, processedAggregatedExpressionHolder.ExpandedCriteria) ? null : ExpandedCriteriaHolder.IfNeededConvertToBoolOperator(processedAggregatedExpressionHolder.ExpandedCriteria));
			return new ExpandedCriteriaHolder(persistentOperator);			
		...
		protected override ExpandedCriteriaHolder Visit(FunctionOperator theOperator) {
			...
			for (int i = 0; i < theOperator.Operands.Count; i++) {
				CriteriaOperator op = theOperator.Operands[i];
			...
			if(result.OperatorType == FunctionOperatorType.Iif) {
				if (holderList.Count < 3 || (holderList.Count % 2) == 0) throw new ArgumentException(Res.GetString(Res.Filtering_TheIifFunctionOperatorRequiresThree));
				if(holderList.Count == 3) {
					if(holderList[0].IsConstant) {
						ExpandedCriteriaHolder firstHolder = Process(ExpandedCriteriaHolder.IfNeededConvertToLogicalOperator(result.Operands[0]));
						if(firstHolder.IsTrue) return holderList[1];
						if(firstHolder.IsFalse) return holderList[2];
					}
					result.Operands[0] = ExpandedCriteriaHolder.IfNeededConvertToLogicalOperator(result.Operands[0]);
					result.Operands[1] = ExpandedCriteriaHolder.IfNeededConvertToBoolOperator(result.Operands[1]);
					result.Operands[2] = ExpandedCriteriaHolder.IfNeededConvertToBoolOperator(result.Operands[2]);
				} else {
					int index = -2;
					int resultShift = 0;
					...
					do {
						if(firstHolder != null){
							if(firstHolder.IsFalse) {
								result.Operands.RemoveRange(index + resultShift, 2);
								resultShift -= 2;
							}
							firstHolder = null;
						}
						index += 2;
						if(holderList[index].IsConstant) {
							firstHolder = Process(ExpandedCriteriaHolder.IfNeededConvertToLogicalOperator(result.Operands[index + resultShift]));
							if(firstHolder.IsTrue){
								if(allAreConstants) return holderList[index + 1];
								result.Operands[index + resultShift] = ExpandedCriteriaHolder.IfNeededConvertToBoolOperator(result.Operands[index + resultShift + 1]);
								result.Operands.RemoveRange(index + resultShift + 1, result.Operands.Count - (index + resultShift + 1));
								cutTail = true;
								break;
							}
						} else {
							allAreConstants = false;
							result.Operands[index + resultShift] = ExpandedCriteriaHolder.IfNeededConvertToLogicalOperator(result.Operands[index + resultShift]);
							result.Operands[index + resultShift + 1] = ExpandedCriteriaHolder.IfNeededConvertToBoolOperator(result.Operands[index + resultShift + 1]);
						}
					} while((index + 3) < holderList.Count);
					if(!cutTail) {
						if(firstHolder != null && firstHolder.IsFalse) {
							if(allAreConstants) return holderList[index + 2];
							result.Operands.RemoveRange(index + resultShift, 2);
							resultShift -= 2;
						}
						result.Operands[index + resultShift + 2] = ExpandedCriteriaHolder.IfNeededConvertToBoolOperator(result.Operands[index + resultShift + 2]);
					}
				}
			} else {
				CriteriaOperator op;
						...
						if(allConstants) {
							...
							if(exactType || instanceOfType) {
								...
								if(propertyClassInfo == null) {
									...
									if(exactType) {
										return IsExactTypeFunction.EqualsType(typeName, objectTypeResult.Type.FullName) ? ExpandedCriteriaHolder.True : ExpandedCriteriaHolder.False;
									}
									return IsInstanceOfTypeFunction.IsInstanceOfType(typeName, objectTypeResult.Type) ? ExpandedCriteriaHolder.True : ExpandedCriteriaHolder.False;
								}
								if(!propertyClassInfo.IsTypedObject) {
									if(exactType) {
										return IsExactTypeFunction.EqualsType(typeName, propertyClassInfo.FullName) ? ExpandedCriteriaHolder.True : ExpandedCriteriaHolder.False;
									}
									return IsInstanceOfTypeFunction.IsInstanceOfType(typeName, propertyClassInfo) ? ExpandedCriteriaHolder.True : ExpandedCriteriaHolder.False;
								}
								...
								if(!searchedClassInfo.IsAssignableTo(propertyClassInfo) || !searchedClassInfo.IsPersistent) return ExpandedCriteriaHolder.False;
							}
						}
						break;
					default:
		...
		protected override ExpandedCriteriaHolder Visit(BinaryOperator theOperator) {
			...
			if(ReferenceEquals(result.LeftOperand, null))
				throw new ArgumentNullException("result.LeftOperand", Xpo.Res.GetString(Xpo.Res.Generator_OneOfBinaryOperatorsOperandsIsNull));
...
#pragma warning restore 618
		...
		OperandValue Evaluate(CriteriaOperator theOperator) {
			ExpressionEvaluator eval = new ExpressionEvaluator(ConstantDescriptor.Instance, theOperator, caseSensitive, Dictionary.CustomFunctionOperators);
...
, WriteToLog
	...
	public class PersistentCriterionExpander : UnknownCriteriaEleminatorBase {
		...
		protected override ExpandedCriteriaHolder Visit(BinaryOperator theOperator) {
			Type leftType = CriteriaTypeResolver.ResolveType(UpLevelsClassInfos, Dictionary, theOperator.LeftOperand, CriteriaTypeResolveKeyBehavior.AlwaysReference);
			Type rightType = CriteriaTypeResolver.ResolveType(UpLevelsClassInfos, Dictionary, theOperator.RightOperand, CriteriaTypeResolveKeyBehavior.AlwaysReference);
		...
		public ExpandedCriteriaHolder ProcessSingleAlias(XPClassInfo[] upLevelsWithReference, CriteriaOperator alias, MemberInfoCollection path, int start) {
			...
			if((!isAggregateOperand && !isJoinOperand) 
				...
				|| (isJoinOperand && joinOperand.AggregateType != Aggregate.Single)) throw new ArgumentException("alias");
		...
		class ManyToManyPrefixer: IClientCriteriaVisitor<CriteriaOperator> {
			...
			public CriteriaOperator Visit(AggregateOperand theOperand) {
				if(theOperand.IsTopLevel) {
					...
					if(cnt > Depth) {
						return new AggregateOperand(processedColectionProperty, theOperand.AggregatedExpression, theOperand.AggregateType, theOperand.Condition);
					} else {
						...
						return new AggregateOperand(processedColectionProperty, prefixedAgg, theOperand.AggregateType, prefixedCond);
					}
				}
			}
			public CriteriaOperator Visit(JoinOperand theOperand) {
				...
				CriteriaOperator prefixedAgg = nestedPrefixer.Process(theOperand.AggregatedExpression);
				CriteriaOperator prefixedCond = nestedPrefixer.Process(theOperand.Condition);
				return new JoinOperand(theOperand.JoinTypeName, prefixedCond, theOperand.AggregateType, prefixedAgg);
			}
			...
			CriteriaOperator[] Process(IList operands) {
		...
		protected static ExpandedCriteriaHolder Expand(XPClassInfo[] upLevels, IPersistentValueExtractor persistentValuesSource, CriteriaOperator op, bool doDetectPostProcessing) {
			...
			if(result.RequiresPostProcessing && PersistentCriterionExpanderRequiresPostProcessingAction != PersistentCriterionExpanderRequiresPostProcessingAction.None) {
				...
				string message = string.Format("Expanding '{0}' criterion for '{1}' classInfo(s) postprocessing was requested because it uses a nonpersistent (and/or modified for NestedUnitOfWork) property '{2}'", op, classInfos, result.PostProcessingCause);
				switch(PersistentCriterionExpanderRequiresPostProcessingAction) {
					...
					case PersistentCriterionExpanderRequiresPostProcessingAction.WriteToConsole:
						Console.WriteLine(message);
						break;
#if !CF && !SL
					case PersistentCriterionExpanderRequiresPostProcessingAction.WriteToLog:
						System.Diagnostics.Trace.WriteLineIf(new System.Diagnostics.TraceSwitch("XPO", "").TraceInfo, message);
						break;
#endif
				}
				return result;
	...
	class TopLevelPropertiesPrefixer: ClientCriteriaVisitorBase {
		...
		protected override CriteriaOperator Visit(OperandProperty theOperand) {
			...
			if(inAggregateOperand == upLevelsPrefixCount) {
				StringBuilder sb = new StringBuilder();
				for(int i = 0; i < upLevelsPrefixCount; i++) {
					sb.Append("^.");
				}
				sb.Append(Prefix);
	...
	public abstract class ContextClientCriteriaVisitorBase<T> : IClientCriteriaVisitor<T> {
		...
		public abstract T VisitInternalJoinOperand(T conditionResult, T agregatedResult);
		...
		public T Visit(JoinOperand theOperand) {
			...
			try {
				...
				try {
					...
					return VisitInternalJoinOperand(conditionResult, agregatedResult);
				} finally {
					currentState.InAtomOperator--;
				}
			} finally {
