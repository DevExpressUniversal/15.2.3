...
namespace DevExpress.Xpo.Helpers {
	...
	public class MergeXmlSerializationHelper {
		...
		static object UpdateObject(UnitOfWork mergeDestination, XPClassInfo ci, object mergeObject, object originalObject, object changedObject) {
			...
			foreach(XPMemberInfo mi in ci.Members) {
				if((mi.IsPersistent || mi.IsAliased) && !mi.IsReadOnly) {
	...
	public class CollectionClonerHelper {
		...
		public static object Clone(Session srcSession, object obj, Session dstSession) {
			if(obj == null)
				return null;
			if(srcSession.IsNewObject(obj, true) || srcSession.IsObjectToSave(obj, true) || srcSession.IsObjectToDelete(obj, true))
	...
	class ClassMetadataHelper {
		...
		static bool IsGoodDefaultProperty(XPMemberInfo mi) {
			...
			if(!mi.IsPublic)
				return false;
#if SL
			if (typeof(DevExpress.Data.Browsing.IBindingList).IsAssignableFrom(mi.MemberType) && !mi.IsCollection)
				return false;
#else
			if(typeof(System.ComponentModel.IBindingList).IsAssignableFrom(mi.MemberType) && !mi.IsCollection)
				return false;
#endif
	...
	public class WebServiceDataStore : SoapHttpClientProtocol, IDataStore, ICommandChannel {
		...
		public UpdateSchemaResult UpdateSchema(bool dontCreateIfFirstTableNotExist, params DBTable[] tables) {
			object[] results = this.Invoke("UpdateSchema", new object[] {
						dontCreateIfFirstTableNotExist, tables});
