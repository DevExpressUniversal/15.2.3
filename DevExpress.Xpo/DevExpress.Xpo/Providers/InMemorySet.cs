...
namespace DevExpress.Xpo.DB.Helpers {
	public class InMemorySet : IXmlSerializable {
		...
		internal bool IsLoadingMode { get { return isLoadingMode; } }
		public InMemorySet()
			: this(true) {
		}
			...
			try {
				for(; ; ) {
					...
					for(; ; ) {
						...
						if(typeStr == null) {
							...
							if(rdr.GetAttribute("base") != "xs:string") { throw new XmlException(Res.GetString(Res.InMemorySet_XMLException_InvalidBaseAttributeValue)); }
							rdr.Read();
							if(rdr.NodeType == XmlNodeType.Whitespace) { rdr.Read(); }
							...
							} else if(typeStr == "xs:string") {
								string dataType = rdr.GetAttribute("DataType", msdata);
								if(!string.IsNullOrEmpty(dataType) && ReferenceEquals(Type.GetType(dataType), typeof(Guid))) {
									type = typeof(Guid);
								} else {
					...
					if(rdr.Name == "xs:keyref") {
						...
						for(int i = 0; i < relations.Count; i++) {
							...
							for(int j = 0; j < indexes.Count; j++) {
								if(indexes[j].Name == relations[i].ReferedIndex) {
									primaryColumns = indexes[j].Fields;
								}
							}
							if(primaryColumns == null) { throw new XmlException(Res.GetString(Res.InMemorySet_XMLException_NullPrimaryColumns, relations[i].ReferedIndex)); }
				...
				while(!rdr.EOF) {
					...
					for(; ; ) {
						...
						Type currentColumnType = currentTable.Columns[columnName].Type;
						if(emptyElement) {
							if(currentColumnType == typeof(string))
								addedRow[columnName] = string.Empty;
							continue;
						}
						addedRow[columnName] = ReadXmlValue(rdr, currentColumnType);
		...
		static object ReadXmlValue(XmlReader rdr, Type currentColumnType) {
			...
			if(typeCode != TypeCode.Object) {
				...
				if(currentColumnType == typeof(TimeSpan)) {
					...
					if(!string.IsNullOrEmpty(xsiType)) {
						rdr.Read();
						if(rdr.NodeType == XmlNodeType.Whitespace) { rdr.Read(); }
						if(xsiType == "xs:base64Binary") {
							return Convert.FromBase64String(rdr.Value);
						} else {
							return ((IConvertible)rdr.Value).ToType(GetTypeFromString(xsiType), CultureInfo.InvariantCulture);
						}
						...
						if(instanceTypeStr == "System.Char") {
							...
							Type instanceType = Type.GetType(instanceTypeStr);
		...
		struct RelationSerializationInfo {
			...
			List<InMemoryColumn> fields;
			public string Name { get { return name; } }
			public string ReferedIndex { get { return referedIndex; } }
			public List<InMemoryColumn> Fields { get { return fields; } }
	...
	public class InMemoryTable : IInMemoryTable {
		...
		public InMemoryTable(InMemorySet set, string name) {
			...
			pRelations = new ReadOnlyCollection<InMemoryRelation>(new InMemoryRelation[0]);
			fRelations = new ReadOnlyCollection<InMemoryRelation>(new InMemoryRelation[0]);
			allRelations = new ReadOnlyCollection<InMemoryRelation>(new InMemoryRelation[0]);
		}
		...
		bool IInMemoryTable.ExistsColumn(string columnName) {
			return Columns.FindColunmIndex(columnName) >= 0;
		}
	...
	public class InMemoryColumnList : IEnumerable<InMemoryColumn> {
		...
		public int FindColunmIndex(string name) {
			int index;
		...
		InMemoryColumn FindColunm(string name) {
			int index = FindColunmIndex(name);
		...
		public void CommitColumnsChanges() {
			foreach(InMemoryColumn currentColumn in columns) {
				...
				if(currentColumn.ColumnForUpdate.Name != currentColumn.Name && FindColunmIndex(currentColumn.ColumnForUpdate.Name) >= 0) throw new InMemoryDuplicateNameException(currentColumn.ColumnForUpdate.Name);
	...
	public class InMemoryColumn {
		...
		InMemoryColumn(InMemoryTable table, string name, Type type, bool autoIncrement, bool allowNull, object defaultValue) {
			...
			if(autoIncrement) {
				...
				if(defaultValue == null) {
					if(!allowNull) throw new InMemoryNoNullAllowedException(string.Format(Res.GetString(Res.InMemorySet_NullDefaultValueNotAllowed), name));
				} else {
			...
			this.pRelations = new ReadOnlyCollection<InMemoryRelation>(new InMemoryRelation[0]);
			this.fRelations = new ReadOnlyCollection<InMemoryRelation>(new InMemoryRelation[0]);
		}
		...
		internal void ColumnsCommited() {
			columnIndex = table.Columns.FindColunmIndex(name);
			Reset();
		}
		...
		public override bool Equals(object obj) {
			...
			return table.Equals(table) && name.Equals(column.name) && type.Equals(type);
		}
		public override int GetHashCode() {
			return table.GetHashCode() ^ name.GetHashCode() ^ type.GetHashCode();
		}
	}
	...
	public class InMemoryDictionaryIndex : IInMemoryIndex {
		...
		public InMemoryDictionaryIndex(InMemoryTable table, InMemoryColumn[] columns, bool unique) {
			if(columns == null || columns.Length == 0) throw new ArgumentException();
			this.table = table;
		...
		public InMemoryRow[] Find(object[] findValues, bool returnDeleted) {
			if(columns.Length != findValues.Length) throw new ArgumentException();
			...
			if(dictionary.TryGetValue(hash, out rowsObject) && rowsObject != null) {
				...
				if(rowInRows != null) {
					if(InMemoryHelper.AreEqualRows(rowInRows, columns, findValues, caseSensitive)
						&& ((returnDeleted) || (!returnDeleted && rowInRows.State != InMemoryItemState.Deleted)))
	...
	public class InMemoryRelation {
		...
		public InMemoryRelation(InMemoryRelationPair[] pairs) {
			if(pairs == null || pairs.Length == 0) throw new ArgumentException();
			this.pairs = pairs;
		...
		public InMemoryRow[] CheckFAssociation(InMemoryColumn col, InMemoryRow row, object newValue) {
			...
			if(InMemoryHelper.IsNullRow(values)) return new InMemoryRow[0];
			if(fCounterDictReady) {
	...
	public class InMemoryComplexSet : IInMemoryDataElector, IEnumerable<InMemoryComplexRow>, IEnumerable {
		...
		struct ProjectionColumnItem {
			public readonly string ProjectionAlias;
			public readonly string ColumnName;
			public ProjectionColumnItem(string projectionAlias, string columnName) {
				if(string.IsNullOrEmpty(projectionAlias)) throw new ArgumentNullException("projectionAlias");
			...
			public override bool Equals(object obj) {
				if(!(obj is ProjectionColumnItem))
					return false;
				ProjectionColumnItem other = (ProjectionColumnItem)obj;
				return ProjectionAlias == other.ProjectionAlias && ColumnName == other.ColumnName;			  
			}
			public override int GetHashCode() {
				return ProjectionAlias.GetHashCode() ^ ColumnName.GetHashCode();
			}
		}
		class ProjectionColumnItemComparer : IEqualityComparer<ProjectionColumnItem> {
			...
			public int GetHashCode(ProjectionColumnItem obj) {
				return obj.ProjectionAlias.GetHashCode() ^ obj.ColumnName.GetHashCode();
			}
		}
	}
