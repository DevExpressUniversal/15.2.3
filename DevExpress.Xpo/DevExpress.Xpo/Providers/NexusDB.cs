...
using System.Reflection;
	public class NexusDBConnectionProvider : ConnectionProviderSql {
		...
		protected override IDataParameter CreateParameter(IDbCommand command, object value, string name) {
			...
			if (name[0]!=':')
				...
				param.ParameterName = name.Substring(1, name.Length-1);
	...
	public class NexusDBProviderFactory : ProviderFactory {
		...
		public override string[] GetDatabases(string server, string userid, string password) {
			Type toolsType = DevExpress.Xpo.Helpers.XPTypeActivator.GetType("NexusDB.ADOProvider", "NexusDB.ADOProvider.NxProviderTools");
			Type connectionType = DevExpress.Xpo.Helpers.XPTypeActivator.GetType("NexusDB.ADOProvider", "NexusDB.ADOProvider.NxConnectionType");
			if(toolsType == null || connectionType == null)
				return new string[0];
			string connection = "Server=" + server + ";UserID=" + userid + ";password=" + password;
