...
namespace DevExpress.Xpo.DB.Helpers {
	...
	public class QueryCriteriaReprocessor : IQueryCriteriaVisitor<CriteriaOperator> {
		...
		NodeInfo[] localRoot = new NodeInfo[0];
	...
	public class DataSetStoreHelpers {
		...
		static string GetRelationName(DataTable table, DBForeignKey fk) {
			...
			name.Append(fk.PrimaryKeyTable);
		...
		static List<DataRow> FindRows(DataViewEvaluatorContextDescriptor descriptor, CriteriaOperator criteria, bool caseSensitive, ICollection<ICustomFunctionOperator> customFunctions, int count) {
			try {
				...
				eval = new ExpressionEvaluator(descriptor, criteria, caseSensitive, customFunctions);
		...
		public static SelectStatementResult DoGetData(DataViewEvaluatorContextDescriptor descriptor, CriteriaOperator condition, ExpressionEvaluator[] dataEvaluators, SortingComparer sortingComparer, int skipRecords, int topRecords, bool caseSensitive, ICollection<ICustomFunctionOperator> customFunction) {
			...
			List<DataRow> rows = DataSetStoreHelpers.FindRows(descriptor, condition, caseSensitive, customFunction, findTopRows);
		...
		public static int DoUpdateRecord(DataViewEvaluatorContextDescriptor descriptor, QueryCriteriaReprocessor processor, QueryParameterCollection parameters, CriteriaOperatorCollection operands, TaggedParametersHolder identitiesByTag, CriteriaOperator condition, bool caseSensitive, ICollection<ICustomFunctionOperator> customFunction, List<DataRow> affectedRows) {
			...
			List<DataRow> rows = FindRows(descriptor, processor.Process(condition), caseSensitive, customFunction, 0);
		...
		public static int DoDeleteRecord(DataViewEvaluatorContextDescriptor descriptor, CriteriaOperator condition, bool caseSensitive, ICollection<ICustomFunctionOperator> customFunctions, List<DataRow> affectedRows) {
			List<DataRow> rows = FindRows(descriptor, condition, caseSensitive, customFunctions, 0);
			int count = rows.Count;
			for(int i = 0; i < count; i++) {
				affectedRows.Add(rows[i]);
				rows[i].Delete();
		...
		public static SelectStatementResult DoGetGroupedData(DataViewEvaluatorContextDescriptor descriptor, CriteriaOperator condition, ExpressionEvaluator[] groupEvaluators, ExpressionEvaluator havingEvaluator, SortingComparer sortingComparer, int skipRecords, int topRecords, ExpressionEvaluator[] dataEvaluators, bool caseSensitive, ICollection<ICustomFunctionOperator> customFunctions) {
			...
			if(groupEvaluators.Length == 0) {
				groupingResult.Add(new InMemoryGroup(new object[] { }), new List<DataRow>());
			}
			int count = rows.Count;
			for(int i = 0; i < count; i++) {
				DataRow row = rows[i];
...
namespace DevExpress.Xpo.DB {
	...
	public class DataSetDataStore : DataStoreBase, IDataStoreSchemaExplorer, IDataStoreSchemaExplorerSp {
		...
		public DataSetDataStore(DataSet data, AutoCreateOption autoCreateOption, bool caseSensitive)
			...
			if(data.HasChanges())
				throw new ArgumentException(Res.GetString(Res.InMemory_DataSetUncommited), "data");
			this.data = data;
		...
		static ExpressionEvaluator PrepareDataEvaluator(QueryCriteriaReprocessor processor, CriteriaOperator operand, EvaluatorContextDescriptor descriptor, bool caseSensitive, ICollection<ICustomFunctionOperator> customFunctions) {
			return new ExpressionEvaluator(descriptor, processor.Process(operand), caseSensitive, customFunctions);
		}
		static ExpressionEvaluator[] PrepareDataEvaluators(QueryCriteriaReprocessor processor, CriteriaOperatorCollection operands, EvaluatorContextDescriptor descriptor, bool caseSensitive, ICollection<ICustomFunctionOperator> customFunctions) {
			int count = operands.Count;
			ExpressionEvaluator[] evaluators = new ExpressionEvaluator[count];
			for(int i = 0; i < count; i++) {
				evaluators[i] = PrepareDataEvaluator(processor, operands[i], descriptor, caseSensitive, customFunctions);
			}
			return evaluators;
		}
		static SortingComparer PrepareSortingComparer(QueryCriteriaReprocessor processor, QuerySortingCollection sortProperties, EvaluatorContextDescriptor descriptor, bool caseSensitive, ICollection<ICustomFunctionOperator> customFunctions) {
			...
			for(int i = 0; i < count; i++) {
				...
				sortingEvaluators[i] = PrepareDataEvaluator(processor, sortingColumn.Property, descriptor, caseSensitive, customFunctions);
			}
		...
		protected SelectStatementResult GetDataNormal(SelectStatement root) {
			...
			ExpressionEvaluator[] dataEvaluators = PrepareDataEvaluators(processor, root.Operands, descriptor, caseSensitive, customFunctionCollection);
			SortingComparer sortingComparer = PrepareSortingComparer(processor, root.SortProperties, descriptor, caseSensitive, customFunctionCollection);
			return DataSetStoreHelpers.DoGetData(descriptor, condition, dataEvaluators, sortingComparer, root.SkipSelectedRecords, root.TopSelectedRecords, caseSensitive, customFunctionCollection);
		}
		protected SelectStatementResult GetDataGrouped(SelectStatement root) {
			...
			CriteriaOperator condition = processor.Process(root.Condition);
			DataViewEvaluatorContextDescriptor descriptor = GetDescriptor(table);
			ExpressionEvaluator[] groupEvaluators = PrepareDataEvaluators(processor, root.GroupProperties, descriptor, caseSensitive, customFunctionCollection);
			EvaluatorContextDescriptor groupedDataDescriptor = new InMemoryGroupDescriptor(descriptor);
			ExpressionEvaluator[] dataEvaluators = PrepareDataEvaluators(processor, root.Operands, groupedDataDescriptor, caseSensitive, customFunctionCollection);
			CriteriaOperatorCollection havingCollection = new CriteriaOperatorCollection();
			ExpressionEvaluator havingEvaluator = PrepareDataEvaluator(processor, root.GroupCondition, groupedDataDescriptor, caseSensitive, customFunctionCollection);
			SortingComparer sortingComparer = PrepareSortingComparer(processor, root.SortProperties, groupedDataDescriptor, caseSensitive, customFunctionCollection);
			return DataSetStoreHelpers.DoGetGroupedData(descriptor, condition, groupEvaluators, havingEvaluator, sortingComparer, root.SkipSelectedRecords, root.TopSelectedRecords, dataEvaluators, caseSensitive, customFunctionCollection);
		}
				...
				for(int i = 0; i < count; i++) {
					...
					try {
						...
						if(root is InsertStatement) {
							...
							recordsUpdated = DataSetStoreHelpers.DoUpdateRecord(GetDescriptor(table), new QueryCriteriaReprocessor(Data, root, identitiesByTag), ((UpdateStatement)root).Parameters, root.Operands, identitiesByTag, root.Condition, caseSensitive, customFunctionCollection, affectedRows);
						} else if(root is DeleteStatement) {
							recordsUpdated = DataSetStoreHelpers.DoDeleteRecord(GetDescriptor(table), new QueryCriteriaReprocessor(Data, root, identitiesByTag).Process(root.Condition), caseSensitive, customFunctionCollection, affectedRows);
						} else {
