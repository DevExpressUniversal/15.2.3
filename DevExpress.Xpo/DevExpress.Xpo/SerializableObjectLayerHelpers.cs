			...
			foreach(XPMemberInfo mi in ci.PersistentProperties) {
				...
				if(mi.IsDelayed && !source.IsNew) {
					if (mi.ReferenceType == null) {
						...
						if (!skipInit) {
							if (!ObjectCollectionLoader.AcceptLoadPropertyAndResetModified(Owner.TrackPropertiesModifications, loadMerge, destination, mi, ReturnArgument, delayedValue))
								continue;
				...
				if(mi.ReferenceType != null && value != null) {
					...
					if(value == null)
						throw new InvalidOperationException(Res.GetString(Res.NestedSession_NotClonable, ci.FullName, mi.Name));	
				}
				if(mi.Converter != null) {
					value = mi.Converter.ConvertFromStorageType(value);
				}
