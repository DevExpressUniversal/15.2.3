	...
	public class Host {
		...
		public string VisibleName {
			get {
				...
				if (Port == -1) return string.Format(CultureInfo.InvariantCulture, "http://{0}/{1}", HostAddress, ServiceName);
				return string.Format(CultureInfo.InvariantCulture, "http://{0}:{1}/{2}", HostAddress, Port, ServiceName);
			}
		}
		public Host() { }
		...
		public override int GetHashCode() {
			return HostAddress.GetHashCode() ^ Port.GetHashCode() ^ Binding.GetHashCode() ^ ServiceName.GetHashCode();
		}
