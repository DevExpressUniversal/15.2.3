...
namespace DevExpress.XtraScheduler.Design {
	public class AppointmentLabelCollectionCodeDomSerializer : CodeDomSerializer {
		...
		public override object Serialize(IDesignerSerializationManager manager, object value) {
			object res = ((CodeDomSerializer)manager.GetSerializer(typeof(AppointmentLabelCollection).BaseType, typeof(CodeDomSerializer))).Serialize(manager, value);
			AppointmentLabelCollection list = value as AppointmentLabelCollection;
			if (list == null)
				return res;
			...
			foreach (AppointmentLabel item in list) {
				Color color = item.Color;
				CodeExpression colorCx = codeDomColorSerializer.Serialize(manager, color) as CodeExpression;
				CodeExpression cx = new CodeMethodInvokeExpression(targetObject, "Add", new CodeExpression[] { colorCx, new CodePrimitiveExpression(item.DisplayName), new CodePrimitiveExpression(item.MenuCaption) });
				if (cx == null)
					return res;
				statements.Add(cx);
			}
