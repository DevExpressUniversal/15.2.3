...
namespace DevExpress.ExpressApp.EF {
	public class EFCollection : ICancelAddNew, IBindingList, IList, ICollection, IEnumerable, ITypedList, IDisposable {
		private EFObjectSpace objectSpace;
		...
		protected void InitObjects() {
			if(objects == null) {
				...
				IQueryable objectsQuery = null;
				if(topReturnedObjectsCount <= 0) {
					objectsQuery = objectSpace.GetObjectsQuery(objectType, memberNames, criteria, sorting);
				}
				else {
					objectsQuery = objectSpace.GetObjectsQuery(objectType, memberNames, criteria, sorting).Take(topReturnedObjectsCount);
				}
				try {
		...
		public EFCollection(EFObjectSpace objectSpace, Type objectType, CriteriaOperator criteria, IList<SortProperty> sorting, Boolean inTransaction) {
			...
			foreach(IMemberInfo memberInfo in EFObjectSpace.GetDefaultDisplayableMembers(propertyDescriptorCollection.TypeInfo)) {
		...
		void ICancelAddNew.CancelNew(Int32 itemIndex) {
			if((newObject != null) && (newObjectIndex == itemIndex)) {
				...
				objectSpace.RemoveFromModifiedObjects(newObject);
				newObject = null;
		...
		Object IBindingList.AddNew() {
			...
			InitObjects();
			newObject = objectSpace.CreateObject(objectType);
