#region Copyright (c) 2000-2015 Developer Express Inc.
/*
{*******************************************************************}
{                                                                   }
{       Developer Express .NET Component Library                    }
{       eXpressApp Framework                                        }
{                                                                   }
{       Copyright (c) 2000-2015 Developer Express Inc.              }
{       ALL RIGHTS RESERVED                                         }
{                                                                   }
{   The entire contents of this file is protected by U.S. and       }
{   International Copyright Laws. Unauthorized reproduction,        }
{   reverse-engineering, and distribution of all or any portion of  }
{   the code contained in this file is strictly prohibited and may  }
{   result in severe civil and criminal penalties and will be       }
{   prosecuted to the maximum extent possible under the law.        }
{                                                                   }
{   RESTRICTIONS                                                    }
{                                                                   }
{   THIS SOURCE CODE AND ALL RESULTING INTERMEDIATE FILES           }
{   ARE CONFIDENTIAL AND PROPRIETARY TRADE                          }
{   SECRETS OF DEVELOPER EXPRESS INC. THE REGISTERED DEVELOPER IS   }
{   LICENSED TO DISTRIBUTE THE PRODUCT AND ALL ACCOMPANYING .NET    }
{   CONTROLS AS PART OF AN EXECUTABLE PROGRAM ONLY.                 }
{                                                                   }
{   THE SOURCE CODE CONTAINED WITHIN THIS FILE AND ALL RELATED      }
{   FILES OR ANY PORTION OF ITS CONTENTS SHALL AT NO TIME BE        }
{   COPIED, TRANSFERRED, SOLD, DISTRIBUTED, OR OTHERWISE MADE       }
{   AVAILABLE TO OTHER INDIVIDUALS WITHOUT EXPRESS WRITTEN CONSENT  }
{   AND PERMISSION FROM DEVELOPER EXPRESS INC.                      }
{                                                                   }
{   CONSULT THE END USER LICENSE AGREEMENT FOR INFORMATION ON       }
{   ADDITIONAL RESTRICTIONS.                                        }
{                                                                   }
{*******************************************************************}
*/
#endregion Copyright (c) 2000-2015 Developer Express Inc.

using System;
using System.Collections.Generic;
using System.ComponentModel;
using DevExpress.ExpressApp.Actions;
using DevExpress.XtraBars;
namespace DevExpress.ExpressApp.Win.Templates.ActionContainers.Items {
	public class BarSimpleButtonItem : BarActionBaseItem
#if DebugTest
, DevExpress.ExpressApp.Tests.ISimpleActionItemUnitTestable, DevExpress.ExpressApp.Tests.IPopupWindowShowActionItemUnitTestable
#endif
 {
		protected override BarItem CreateControlCore() {
			return new BarButtonItem();
		}
		protected override void ItemClicked(BarItemLink barItemLink) {
			base.ItemClicked(barItemLink);
			if(IsItemClickEnable && IsConfirmed()) {
				ResetLastItemClickTime();
				if(Action is SimpleAction) {
					((SimpleAction)Action).DoExecute();
				}
				else if(Action is PopupWindowShowAction) {
					ShowPopupWindow((PopupWindowShowAction)Action);
				}
			}
		}
		protected virtual void ShowPopupWindow(PopupWindowShowAction action) {
			using(PopupWindowShowActionHelper helper = new PopupWindowShowActionHelper(action)) {
				helper.ShowPopupWindow();
			}
		}
		public BarSimpleButtonItem(ActionBase action, BarManager manager) : base(action, manager) { }
		#region Obsolete 14.2
		[Obsolete(ObsoleteMessages.MemberIsNotUsedAnymore), Browsable(false), EditorBrowsable(EditorBrowsableState.Never)]
		public void Execute() {
			ItemClicked(null);
		}
		[Obsolete(ObsoleteMessages.MemberIsNotUsedAnymore, true), Browsable(false), EditorBrowsable(EditorBrowsableState.Never)]
		protected List<IDisposable> OpenedWindows {
			get { throw new InvalidOperationException(); }
		}
		#endregion
	}
}
