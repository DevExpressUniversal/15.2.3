...
namespace DevExpress.ExpressApp.Win.Editors {
	public class XafBandedGridView : AdvBandedGridView, IModelSynchronizersHolder, IXafGridView, ISupportNewItemRow, ICustomizeFilterColumns {
		...
		protected override BaseView CreateInstance() {
			XafBandedGridView view = CreateInstanceCore();
			view.SetGridControl(GridControl);
			return view;
		}
		protected virtual XafBandedGridView CreateInstanceCore() {
			return new XafBandedGridView();
		}
		...
		void IModelSynchronizersHolder.AssignSynchronizers(ColumnView sourceView) {
			...
			IModelSynchronizersHolder modelSynchronizersHolder = (IModelSynchronizersHolder)sourceView;
			for(int n = 0; n < sourceView.Columns.Count; n++) {
				IModelSynchronizer info = modelSynchronizersHolder.GetSynchronizer(sourceView.Columns[n]);
				if(info != null) {
					current.RegisterSynchronizer(Columns[n], info);
				}
			}
			AssignBandsSynchronizers(sourceView as BandedGridView, modelSynchronizersHolder);
		}
		private void AssignBandsSynchronizers(BandedGridView sourceView, IModelSynchronizersHolder sourceModelSynchronizersHolder) {
			if(sourceView != null) {
				AssignBandsSynchronizers(sourceView.Bands, Bands, sourceModelSynchronizersHolder);
			}
		}
		private void AssignBandsSynchronizers(GridBandCollection sourcebands, GridBandCollection currentBands, IModelSynchronizersHolder sourceModelSynchronizersHolder) {
			IModelSynchronizersHolder current = (IModelSynchronizersHolder)this;
			for(int n = 0; n < sourcebands.Count; n++) {
				IModelSynchronizer info = sourceModelSynchronizersHolder.GetSynchronizer(sourcebands[n]);
				if(info != null) {
					current.RegisterSynchronizer(currentBands[n], info);
				}
				AssignBandsSynchronizers(sourcebands[n].Children, currentBands[n].Children, sourceModelSynchronizersHolder);
			}
		}
