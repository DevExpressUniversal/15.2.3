...
namespace DevExpress.ExpressApp.Web.Editors {
	public class WebCustomUserControlViewItem : ViewItem, IComplexViewItem {
		IModelCustomUserControlViewItemWeb model;
		...
		public WebCustomUserControlViewItem(IModelViewItem model, Type objectType)
			...
			this.model = (IModelCustomUserControlViewItemWeb)model;
		}
		protected override object CreateControlCore() {
			System.Web.UI.Control userControl = WebWindow.CurrentRequestPage.LoadControl(model.CustomControlPath);
			if(userControl != null) {
				if(userControl is IComplexControl) {
					((IComplexControl)userControl).Setup(objectSpace, application);
				}
			}
			return userControl;
		}
