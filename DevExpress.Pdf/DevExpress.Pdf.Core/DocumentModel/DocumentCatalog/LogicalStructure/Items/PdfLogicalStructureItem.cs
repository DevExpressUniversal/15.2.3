...
namespace DevExpress.Pdf {
	public abstract class PdfLogicalStructureItem : PdfObject {
		internal static PdfLogicalStructureItem Parse(PdfLogicalStructure logicalStructure, PdfLogicalStructureEntry parent, object value) {
			PdfLogicalStructureElement parentElement = parent as PdfLogicalStructureElement;
			PdfReaderDictionary dictionary = value as PdfReaderDictionary;
			if (dictionary == null) {
				if (parentElement == null || !(value is int))
					PdfDocumentReader.ThrowIncorrectDataException();
				return new PdfLogicalStructureMcidContent((int)value);
			}
			switch (dictionary.GetName(PdfDictionary.DictionaryTypeKey)) {
				case PdfLogicalStructureMarkedContentReference.Type:
					if (parentElement == null)
						PdfDocumentReader.ThrowIncorrectDataException();
					return new PdfLogicalStructureMarkedContentReference(dictionary);
				case PdfLogicalStructureContentItem.Type:
					if (parentElement == null)
						PdfDocumentReader.ThrowIncorrectDataException();
					PdfLogicalStructureContentItem contentItem = new PdfLogicalStructureContentItem(parentElement.Page, dictionary);
					return contentItem.Content == null ? null : contentItem;
				default:
					return new PdfLogicalStructureElement(logicalStructure, parent, dictionary);
			}
		}
		protected PdfLogicalStructureItem(int objectNumber) : base(objectNumber) { 
		}
		protected internal virtual PdfPage ContainingPage { get { return null; } }
		...
		protected internal virtual object Write(PdfObjectCollection collection) {
			return collection.AddObject(this);
		}
