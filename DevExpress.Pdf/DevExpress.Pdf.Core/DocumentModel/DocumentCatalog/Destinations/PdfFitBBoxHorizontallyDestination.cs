...
namespace DevExpress.Pdf {
	public class PdfFitBBoxHorizontallyDestination : PdfDestination {
		...
		PdfFitBBoxHorizontallyDestination(PdfFitBBoxHorizontallyDestination destination)
			: base(destination) {
			top = destination.top;
		}
		protected internal override PdfDestination CreateDuplicate() {
			return new PdfFitBBoxHorizontallyDestination(this);
		}
		protected internal override PdfTarget CreateTarget(IList<PdfPage> pages) {
			return new PdfTarget(PdfTargetMode.FitBBoxHorizontally, CalculatePageIndex(pages), null, ValidateVerticalCoordinate(top));
		}
