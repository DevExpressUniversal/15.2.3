...
namespace DevExpress.Pdf {
	public class PdfFitVerticallyDestination : PdfDestination {
		...
		PdfFitVerticallyDestination(PdfFitVerticallyDestination destination)
			: base(destination) {
			left = destination.left;
		}
		protected internal override PdfDestination CreateDuplicate() {
			return new PdfFitVerticallyDestination(this);
		}
		protected internal override PdfTarget CreateTarget(IList<PdfPage> pages) {
			return new PdfTarget(PdfTargetMode.FitVertically, CalculatePageIndex(pages), left, null);
		}
