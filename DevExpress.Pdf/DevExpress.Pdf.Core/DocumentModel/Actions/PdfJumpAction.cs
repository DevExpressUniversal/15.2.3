...
namespace DevExpress.Pdf {
	public abstract class PdfJumpAction : PdfAction {
		const string destinationDictionaryKey = "D";
		readonly PdfDestinationObject destination;
		protected abstract bool IsInternal { get; }
		public PdfDestination Destination { get { return destination == null ? null : destination.GetDestination(DocumentCatalog, IsInternal); } }
		...
		protected override PdfWriterDictionary CreateDictionary(PdfObjectCollection objects) {
			...
			if (destination != null)
				dictionary.Add(destinationDictionaryKey, destination.ToWriteableObject(DocumentCatalog, objects, IsInternal));
			return dictionary;
		}
	}
}
