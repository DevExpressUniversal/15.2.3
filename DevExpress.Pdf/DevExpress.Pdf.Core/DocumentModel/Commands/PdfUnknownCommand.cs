...
namespace DevExpress.Pdf {
	public class PdfUnknownCommand : PdfCommand {
		...
		public IList<object> Parameters { get { return parameters; } }
		internal PdfUnknownCommand(string name, PdfOperands operands) {
			this.name = name;
			int count = operands.Count;
			for (int i = 0; i < count; i++)
				parameters.Add(operands.PopObject());
		}
		protected internal override void Write(PdfResources resources, PdfDocumentWritableStream writer) {
			foreach (object param in parameters) {
				writer.WriteSpace();
				writer.WriteObject(param, -1);
			}
			writer.WriteSpace();
