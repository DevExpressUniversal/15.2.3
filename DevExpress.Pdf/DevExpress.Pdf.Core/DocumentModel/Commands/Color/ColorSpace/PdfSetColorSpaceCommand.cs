...
namespace DevExpress.Pdf {
	public abstract class PdfSetColorSpaceCommand : PdfCommand {
		...
		protected PdfSetColorSpaceCommand(PdfResources resources, PdfOperands operands) {
			colorSpace = resources.GetColorSpace(operands.PopName());
			if (colorSpace == null)
				PdfDocumentReader.ThrowIncorrectDataException();
		}
		protected internal override void Write(PdfResources resources, PdfDocumentWritableStream writer) {
			writer.WriteSpace();
			writer.WriteObject(resources.FindColorSpaceName(colorSpace) ?? colorSpace.Write(null), -1);
			writer.WriteSpace();
		}
	}
}
