...
namespace DevExpress.Pdf {
	public class PdfSetFlatnessToleranceCommand : PdfCommand {
		...
		static bool ValidateFlatnessTolerance(double flatnessTolerance) {
			return flatnessTolerance >= 0 && flatnessTolerance <= 100;
		}
		readonly double flatnessTolerance;
		public double FlatnessTolerance { get { return flatnessTolerance; } }
		...
		internal PdfSetFlatnessToleranceCommand(PdfOperands operands) {
			flatnessTolerance = operands.PopDouble();
			if (!ValidateFlatnessTolerance(flatnessTolerance))
				PdfDocumentReader.ThrowIncorrectDataException();
		}
		protected internal override void Write(PdfResources resources, PdfDocumentWritableStream writer) {
			writer.WriteSpace();
			writer.WriteDouble(flatnessTolerance);
			writer.WriteSpace();
			writer.WriteString(Name);
		}
		protected internal override void Execute(PdfCommandInterpreter interpreter) {
			interpreter.SetFlatnessTolerance(flatnessTolerance);
		}
	}
}
