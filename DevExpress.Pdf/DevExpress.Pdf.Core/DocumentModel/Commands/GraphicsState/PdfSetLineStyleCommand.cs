...
namespace DevExpress.Pdf {
	public class PdfSetLineStyleCommand : PdfCommand {
		...
		internal PdfSetLineStyleCommand(PdfOperands operands) {
			lineStyle = PdfLineStyle.Parse(new object[] { operands.PopObject(), operands.PopObject() });
		}
		protected internal override void Write(PdfResources resources, PdfDocumentWritableStream writer) {
			foreach (object value in lineStyle.Data) {
				writer.WriteSpace();
				writer.WriteObject(value, -1);
			}
			writer.WriteSpace();
			writer.WriteString(Name);
		}
		protected internal override void Execute(PdfCommandInterpreter interpreter) {
			interpreter.SetLineStyle(lineStyle);
		}
	}
}
