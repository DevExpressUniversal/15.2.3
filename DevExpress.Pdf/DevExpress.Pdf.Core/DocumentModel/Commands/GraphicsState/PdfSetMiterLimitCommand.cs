...
namespace DevExpress.Pdf {
	public class PdfSetMiterLimitCommand : PdfCommand {
		...
		readonly double miterLimit;
		public double MiterLimit { get { return miterLimit; } }
		...
		internal PdfSetMiterLimitCommand(PdfOperands operands) {
			miterLimit = operands.PopDouble();
			if (miterLimit <= 0)
				PdfDocumentReader.ThrowIncorrectDataException();
		}
		protected internal override void Write(PdfResources resources, PdfDocumentWritableStream writer) {
			writer.WriteSpace();
			writer.WriteDouble(miterLimit);
			writer.WriteSpace();
			writer.WriteString(Name);
		}
		protected internal override void Execute(PdfCommandInterpreter interpreter) {
			interpreter.SetMiterLimit(miterLimit);
		}
	}
}
