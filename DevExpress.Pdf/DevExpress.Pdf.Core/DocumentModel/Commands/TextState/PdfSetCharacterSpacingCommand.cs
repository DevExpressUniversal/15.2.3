...
namespace DevExpress.Pdf {
	public class PdfSetCharacterSpacingCommand : PdfCommand {
		...
		internal PdfSetCharacterSpacingCommand(PdfOperands operands) {
			characterSpacing = operands.PopDouble();
		}
		protected internal override void Write(PdfResources resources, PdfDocumentWritableStream writer) {
			writer.WriteSpace();
			writer.WriteDouble(characterSpacing);
			writer.WriteSpace();
			writer.WriteString(Name);
		}
		protected internal override void Execute(PdfCommandInterpreter interpreter) {
			interpreter.SetCharacterSpacing(characterSpacing);
		}
	}
}
