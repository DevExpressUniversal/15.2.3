...
namespace DevExpress.Pdf {
	public class PdfSetTextRiseCommand : PdfCommand {
		...
		readonly double textRise;
		public double TextRise { get { return textRise; } }
		public PdfSetTextRiseCommand(double textRise) {
			this.textRise = textRise;
		}
		internal PdfSetTextRiseCommand(PdfOperands operands) {
			textRise = operands.PopDouble();
		}
		protected internal override void Execute(PdfCommandInterpreter interpreter) {
			interpreter.SetTextRise(textRise);
		}
		protected internal override void Write(PdfResources resources, PdfDocumentWritableStream writer) {
			writer.WriteSpace();
			writer.WriteDouble(textRise);
			writer.WriteSpace();
