...
namespace DevExpress.Pdf {
	public class PdfSetTextLeadingCommand : PdfCommand {
		...
		readonly double textLeading;
		public double TextLeading { get { return textLeading; } }
		public PdfSetTextLeadingCommand(double textLeading) {
			this.textLeading = textLeading;
		}
		internal PdfSetTextLeadingCommand(PdfOperands operands) {
			textLeading = operands.PopDouble();
		}
		protected internal override void Write(PdfResources resources, PdfDocumentWritableStream writer) {
			writer.WriteSpace();
			writer.WriteDouble(textLeading);
			writer.WriteSpace();
			writer.WriteString(Name);
		}
		protected internal override void Execute(PdfCommandInterpreter interpreter) {
			interpreter.SetTextLeading(textLeading);
		}
	}
}
