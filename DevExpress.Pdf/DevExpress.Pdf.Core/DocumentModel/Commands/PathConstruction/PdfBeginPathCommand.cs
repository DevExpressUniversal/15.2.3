...
namespace DevExpress.Pdf {
	public class PdfBeginPathCommand : PdfCommand {
		...
		internal PdfBeginPathCommand(PdfOperands operands) {
			startPoint = new PdfPoint(operands);
		}
		protected internal override void Write(PdfResources resources, PdfDocumentWritableStream writer) {
			writer.WriteSpace();
			writer.WriteDouble(startPoint.X);
			writer.WriteSpace();
			writer.WriteDouble(startPoint.Y);
			writer.WriteSpace();
			writer.WriteString(Name);
		}
		protected internal override void Execute(PdfCommandInterpreter interpreter) {
			interpreter.BeginPath(startPoint);
		}
	}
}
