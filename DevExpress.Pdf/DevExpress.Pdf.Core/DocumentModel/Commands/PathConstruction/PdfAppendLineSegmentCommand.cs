...
namespace DevExpress.Pdf {
	public class PdfAppendLineSegmentCommand : PdfCommand {
		...
		internal PdfAppendLineSegmentCommand(PdfOperands operands) {
			endPoint = new PdfPoint(operands);
		}
		protected internal override void Write(PdfResources resources, PdfDocumentWritableStream writer) {
			writer.WriteSpace();
			writer.WriteDouble(endPoint.X);
			writer.WriteSpace();
			writer.WriteDouble(endPoint.Y);
			writer.WriteSpace();
			writer.WriteString(Name);
		}
		protected internal override void Execute(PdfCommandInterpreter interpreter) {
			interpreter.AppendPathLineSegment(endPoint);
		}
	}
}
