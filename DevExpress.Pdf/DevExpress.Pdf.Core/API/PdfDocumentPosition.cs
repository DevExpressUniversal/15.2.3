...
namespace DevExpress.Pdf {
	public class PdfDocumentPosition {
		const int nearDistance = 3;
		readonly int pageNumber;
		readonly PdfPoint point;
		internal int PageIndex { get { return pageNumber - 1; } }
		public int PageNumber { get { return pageNumber; } }
		public PdfPoint Point { get { return point; } }
		public PdfDocumentPosition(int pageNumber, PdfPoint point) {
			this.pageNumber = pageNumber;
			this.point = point;
		}
		internal bool NearTo(PdfDocumentPosition position) {
			PdfPoint otherPoint = position.point;
			return pageNumber == position.pageNumber && Math.Abs(point.X - otherPoint.X) <= nearDistance && Math.Abs(point.Y - otherPoint.Y) <= nearDistance;
		}
	}
}
