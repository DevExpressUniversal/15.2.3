...
namespace DevExpress.Pdf.Native {
	public class PdfFontCmapGroup {
		public static PdfFontCmapGroup[] ReadGroups(PdfBinaryStream stream, int groupsCount) {
			PdfFontCmapGroup[] groups = new PdfFontCmapGroup[groupsCount];
			for (int i = 0; i < groupsCount; i++) {
				int startCharCode = stream.ReadInt();
				int endCharCode = stream.ReadInt();
				int glyphID = stream.ReadInt();
				groups[i] = new PdfFontCmapGroup(startCharCode, endCharCode, glyphID);
			}
			return groups;
		}
		...
		readonly int glyphID;
		public int StartCharCode { get { return startCharCode; } }
		public int EndCharCode { get { return endCharCode; } }
		public int GlyphID { get { return glyphID; } }
