...
namespace DevExpress.Pdf.Native {
	public class PdfPostScriptType1FontOperator : PdfPostScriptOperator {
		...
		public PdfPostScriptType1FontOperator(string token) {
			this.token = token;
		}
		public override void Execute(PdfPostScriptInterpreter interpreter) {
			...
			PdfPostScriptDictionary dictionary = dictionaryStack.Peek();
			if (!dictionary.ContainsKey(token))
				PdfDocumentReader.ThrowIncorrectDataException();
			IList<object> procedure = dictionary[token] as IList<object>;
			if (procedure == null)
				PdfDocumentReader.ThrowIncorrectDataException();
