...
namespace DevExpress.Internal {
	public class DataDirectoryHelper {
		...
		public static string GetFile(string relativePath, string rootPath) {
			if(DataPath != null) {
			...
			return GetPath(relativePath, rootPath);
		}
		static void ThrowNotFound(string path) {
			string message = string.Format("{0} not found. If the demo has been moved to a new location, make sure the Data directory is there as well.", path);
			throw new FileNotFoundException(message);
		}
		static string GetPath(string fileName, string directoryName) {
			string dataDirectory = GetDataDirectory();
			if(dataDirectory == null) return null;
			string dirName = Path.GetFullPath(dataDirectory);
			for(int n = 0; n < 9; n++) {
				string path = dirName + "\\" + directoryName + (fileName != null ? "\\" + fileName : "");
				try {
			...
			ThrowNotFound(string.Format("{0} ({1})", fileName, dirName));
			throw new NotImplementedException();
		}
		public static string GetDirectory(string directoryName) {
			return GetPath(null, directoryName);
		}
#if !REALTORWORLDDEMO
		...
		static string GetEntryAssemblyDirectory() {
			...
			if(entryAssembly == null) return AppDomain.CurrentDomain.BaseDirectory;
