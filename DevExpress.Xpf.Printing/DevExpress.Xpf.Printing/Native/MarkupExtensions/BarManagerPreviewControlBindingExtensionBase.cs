...
namespace DevExpress.Xpf.Printing {
	public abstract class BarManagerPreviewControlBindingExtensionBase : FindAncestorBindingExtension {
		protected override Type AncestorType {
			get { return typeof(BarManager); }
		}
		protected override PropertyPath BindingPath {
			get {
				if(string.IsNullOrEmpty(Path))
					return new PropertyPath(ControlProperty);
#if SL
				return new PropertyPath(string.Format("Controllers[0].DocumentViewer.{0}", Path));
#else
				return new PropertyPath("(0)." + Path, ControlProperty);
#endif
			}
		}
