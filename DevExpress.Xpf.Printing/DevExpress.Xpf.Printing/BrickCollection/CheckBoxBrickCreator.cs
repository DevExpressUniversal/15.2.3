...
namespace DevExpress.Xpf.Printing.BrickCollection {
	class CheckBoxBrickCreator : BrickCreator {
		public CheckBoxBrickCreator(PrintingSystemBase ps, Dictionary<BrickStyleKey, BrickStyle> brickStyles, Dictionary<IVisualBrick, IOnPageUpdater> onPageUpdaters)
		...
		public override VisualBrick Create(UIElement source, UIElement parent) {
			...
			brick.CheckState = ConvertToCheckState(exportSettings.BooleanValue);
			brick.CheckText = exportSettings.CheckText;
			return brick;
		}
		CheckState ConvertToCheckState(bool? value) {
			return !value.HasValue ? CheckState.Indeterminate :
