...
namespace DevExpress.XtraSpreadsheet.Model.CopyOperation {
	public class PivotTableProcessorForRangeCutOperation : PivotTableProcessorForRangeCopyOperation  {
		public PivotTableProcessorForRangeCutOperation(ITargetRangeCalculatorOwner rangeCopyOperation, IRangeCopyPivotTablePart rangeCopy)
			: base(rangeCopyOperation, rangeCopy) {
		}
		#region Calculate
		public override void Calculate(RangeCopyInfo rangeInfo, PivotTable sourceTable, IList<string> existingTableNames) {
			...
			if (sourceRange.Includes(sourceObjectRange)) {
				...
				if (canTableInsert)
					RangeCopy.ChangeExistingPivotTableRange(sourceTable, targetObjectRange);
				else {
				}
			}
		}
