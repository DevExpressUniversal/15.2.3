...
namespace DevExpress.XtraSpreadsheet.Model {
	#region FunctionNominal
	public class FunctionNominal : WorksheetFunctionBase {
		static readonly FunctionParameterCollection functionParameters = PrepareParameters();
		public override string Name { get { return "NOMINAL"; } }
		public override int Code { get { return 0x01BD; } }
		...
		protected override VariantValue EvaluateCore(IList<VariantValue> arguments, WorkbookDataContext context) {
			...
			if (value.IsError)
				return value;
			double effectRate = value.NumericValue;
			value = GetNumberValue(arguments[1], context);
			if (value.IsError)
				return value;
			int npery = (int)value.NumericValue;
			if (effectRate <= 0 || npery < 1)
				return VariantValue.ErrorNumber;
			return npery * (Math.Pow(effectRate + 1, 1.0 / (double)npery) - 1);
		}
