...
namespace DevExpress.XtraSpreadsheet.Model {
	#region FunctionIfNA
	public class FunctionIfNA : FunctionIfBase {
		static readonly FunctionParameterCollection functionParameters = PrepareParameters();
		public override string Name { get { return "IFNA"; } }
		public override int Code { get { return 0x403C; } }
		...
		VariantValue EvaluateArrayValue(IVariantArray array, VariantValue valueIfError, WorkbookDataContext context) {
			...
			for (int i = 0; i < height; i++) {
				for (int j = 0; j < width; j++) {
					...
					if (!(conditionValue == VariantValue.ErrorValueNotAvailable))
