...
namespace DevExpress.XtraSpreadsheet.Model {
	...
	public class FunctionColumn : WorksheetFunctionBase {
		static readonly FunctionParameterCollection functionParameters = PrepareParameters();
		public override string Name { get { return "COLUMN"; } }
		public override int Code { get { return 0x0009; } }
		...
		protected virtual VariantValue EvaluateArgument(VariantValue argument, WorkbookDataContext context) {
			...
			int leftColumn = cellRange.TopLeft.Column + 1;
			int width = cellRange.Width;
			if (width > 1) {
				VariantArray resultArray = VariantArray.Create(width, 1);
				for (int i = 0; i < width; i++)
					resultArray.Values[i] = leftColumn + i;
