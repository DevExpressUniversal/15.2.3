...
namespace DevExpress.XtraSpreadsheet.Model {
	#region FunctionSheets
	public class FunctionSheets : WorksheetFunctionSingleArgumentBase {
		static readonly FunctionParameterCollection functionParameters = PrepareParameters();
		public override string Name { get { return "SHEETS"; } }
		public override int Code { get { return 0x4041; } }
		...
		protected override VariantValue EvaluateArgument(VariantValue argument, WorkbookDataContext context) {
			if (argument.IsError)
				return argument;
			if (argument.IsEmpty)
				return context.Workbook.Sheets.Count;
			if (argument.IsNumeric)
				return VariantValue.ErrorValueNotAvailable;
			CellRangeBase range = argument.CellRangeValue;
			if (range == null)
			...
			if (range.RangeType == CellRangeType.UnionRange) {
				...
				foreach (CellRangeBase innerRange in union.InnerCellRanges) 
					if (!collection.ContainsKey(innerRange.Worksheet.Name))
						collection.Add(innerRange.Worksheet.Name, 0);
