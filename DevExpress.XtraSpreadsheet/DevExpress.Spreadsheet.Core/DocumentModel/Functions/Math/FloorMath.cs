...
namespace DevExpress.XtraSpreadsheet.Model {
	#region FunctionFloorMath
	public class FunctionFloorMath : WorksheetFunctionBase {
		static readonly FunctionParameterCollection functionParameters = PrepareParameters();
		public override string Name { get { return "FLOOR.MATH"; } }
		public override int Code { get { return 0x4020; } }
		...
		double GetNumericResult(double number, double significance, double mode) {
			if (significance == 0 || number == 0)
				return 0;
			significance = Math.Abs(significance);
			double result = Math.Abs(number) / significance;
			if (result - Math.Ceiling(result) == 0)
				result = number;
			else {
