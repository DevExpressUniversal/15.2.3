...
namespace DevExpress.XtraSpreadsheet {
	public class OneDocumentMailmergeProcessor :MailMergeProcessor {
		bool unlockFinishList;
		public OneDocumentMailmergeProcessor(Worksheet templateSheet, MailMergeOptions options)
		...
		public override List<DocumentModel> Process() {
			DocumentModel targetBook = new DocumentModel();
			targetBook.MailMergeProcessor = this;
			DetectedTrackedPositions();
			while (!DataAdapter.LastRowProcessed) {
				Worksheet targetSheet = new Worksheet(targetBook);
				targetBook.Sheets.Add(targetSheet);
				GenerateContent(targetSheet);
			}
			return new List<DocumentModel> { targetBook };
		}
