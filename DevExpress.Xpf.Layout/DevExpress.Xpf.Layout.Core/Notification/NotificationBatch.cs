...
namespace DevExpress.Xpf.Layout.Core {
	public class NotificationBatch : IDisposable {
		...
		public static void Updating(object source) {
			...
			lock(syncObj) {
				int value;
				if(!lockHash.TryGetValue(source, out value)) {
		...
		public static void Action(object source, object target = null, DependencyProperty property = null) {
			NotificationHelper.Notify(source,
					new NotificationEventArgs(Notification.Action) { ActionTarget = target, Property = property }
				);
		}
	}
}
