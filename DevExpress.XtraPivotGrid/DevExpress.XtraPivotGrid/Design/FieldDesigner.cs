...
namespace DevExpress.XtraPivotGrid.Design.Frames {
	...
	public class FieldDesigner : DevExpress.XtraEditors.Frames.ColumnDesignerBase	{
		...
		protected override bool CanRetrieveColumn(string fieldName) {
			if(PivotGrid == null)
				return false;
			if(PivotGrid.IsOLAPDataSource && !IsMeasureFieldName(fieldName) && PivotGrid.Fields[fieldName] != null)
				return false;
			return base.CanRetrieveColumn(fieldName);
		}
		...
		protected override object CreateNewColumn(string fieldName, int index) {
			...
			PivotArea area = PivotArea.ColumnArea;
			if(PivotGrid.IsOLAPDataSource && IsMeasureFieldName(fieldName))
				area = PivotArea.DataArea;
			PivotGridField field = PivotGrid.Fields.Add(fieldName, area);
			field.Index = index;
			return field;
		}
		bool IsMeasureFieldName(string fieldName) {
			return fieldName.ToUpper().Contains("[MEASURES]");
		}
	...
	public class PivotGridFieldObject : ColumnObject {
		...
		public override bool Visible { get { return Field.Visible; } }
