...
namespace DevExpress.PivotGrid.Xmla {
	class XmlaCellSetParser : OLAPCellSetParser {
		internal XmlaCellSetParser(IOLAPMetadata owner)
			: base(owner) {
		}
		public override IOLAPRowSet QueryDrillDown(IOLAPCommand command) {
			return ((XmlaCommand)command).ExecuteRowSet();
		}
		protected override IOLAPCellSet ExecuteCellSet(IOLAPCommand command, IQueryContext<OLAPCubeColumn> context) {
			return ((XmlaCommand)command).ExecuteCellSet(context != null ? (OLAPAreas)context.Areas : null);
		}
	}
}
