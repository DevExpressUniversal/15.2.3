...
namespace DevExpress.Data.PivotGrid {
	public class PivotFileDataSourceHelper {
		...
		public static void CopyData(Stream src, Stream dest) {
	...
	public class PivotFileDataSource : ITypedList, IList {
		...
		protected internal DataStorageObjectComparer[] Storage { get { return storage; } }
		...
		public PivotFileDataSource(ICustomObjectConverter customObjectConverter) {
			this.customObjectConverter = customObjectConverter;
		}
		public PivotFileDataSource(string fileName) : this(fileName, null) { }
		public PivotFileDataSource(string fileName, ICustomObjectConverter customObjectConverter) {
			this.customObjectConverter = customObjectConverter;
			this.FileName = fileName;
		}
		public PivotFileDataSource(Stream stream) : this(stream, null) { }
		public PivotFileDataSource(Stream stream, ICustomObjectConverter customObjectConverter) {
			this.customObjectConverter = customObjectConverter;
			this.stream = stream;
			LoadStream(stream);
		}
				...
				case 2:
					using(MemoryStream columnsStream = new MemoryStream()) {
						using(DeflateStream decompressStream = new DeflateStream(stream, CompressionMode.Decompress, true)) {
							PivotFileDataSourceHelper.CopyData(decompressStream, columnsStream);
							columnsStream.Position = 0;
						}						
						LoadStorageColumnsCore(columnsStream);
					}
					break;
				case 1:
		...
		void LoadStorageColumnsCore(Stream stream) {
			...
			for(int i = 0; i < storageCount; i++) {
				string sign = reader.ReadString();
				if(sign == DataStorageObjectComparer.NullStreamSign)
					storage[i] = null;
				else {
					if(sign != DataStorageObjectComparer.StreamSign) 
						throw new Exception("Corrupted data!");
					DataStorageObjectComparer storageComparer = DataStorageObjectComparer.CreateComparer(ItemProperties[i].PropertyType);
					storageComparer.CustomObjectConverter = customObjectConverter;
					storageComparer.LoadFromStream(stream, false);
					storageComparer.SetCustomComparer(new PivotStorageObjectValueComparer(true));
					storage[i] = storageComparer;
				}
			}
		}
#region ITypedList Members
		...
		internal object GetRowValue(int rowIndex, int columnIndex) {
			if(rowIndex >= 0 && rowIndex < RowCount && columnIndex >= 0 && columnIndex < Storage.Length)
				return Storage[columnIndex] != null ? Storage[columnIndex].GetNullableRecordValue(rowIndex) : null;
			return null;
		}
#endregion
