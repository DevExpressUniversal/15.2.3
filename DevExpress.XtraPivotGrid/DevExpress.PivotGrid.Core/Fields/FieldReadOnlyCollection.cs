...
namespace DevExpress.XtraPivotGrid.Data {
	...
	public class PivotGridFieldReadOnlyCollection : CollectionBase {
		...
		public void Sort() {
			InnerList.Sort(new PivotGridFieldAreaIndexComparer());
		}
		...
		public delegate bool FindPredicate(PivotGridFieldBase field);
		public List<PivotGridFieldBase> FindAll(FindPredicate predicate) {
			List<PivotGridFieldBase> res = null;
			for(int i = 0; i < Count; i++) {
				PivotGridFieldBase field = this[i];
				if(!predicate(field)) continue;
				if(res == null)
					res = new List<PivotGridFieldBase>();
				res.Add(field);
			}
			return res;
		}
