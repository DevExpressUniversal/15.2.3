...
namespace DevExpress.Design.DataAccess {
	class XmlDataSetSourceGenerator : DataSourceGeneratorBase {
		...
		protected sealed override bool GenerateCodeBehind(IModelItem dataSourceItem, IDataSourceGeneratorContext context) {
			context.GenerateCode(GenerateReadXmlExpression(dataSourceItem, context));
			context.GenerateDataSourceAssignment(context.GenerateExpression(
				new object[] { GetElementName(context) }, "xmlDataSet.Tables[\"{0}\"]"));
			return true;
		}
	}
}
